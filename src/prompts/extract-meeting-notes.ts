...
- A header or footer line that names them — "Pengerusi: X  Setiausaha: Y", 「主席：甲　记录：乙」, a signature block — IS such a list: X and Y go into "office_bearers" with those positions, EVERY time. Filling "prepared_by" / "endorsed_by" from the same line is an extra COPY for the signature block and never removes anyone from "office_bearers".
...
Figure "role" — what each amount IS in the treasurer's report, so our code can check the arithmetic (you never do it): "opening" = the balance brought forward (上年结存 / 上期结存 / baki tahun lepas / baki bawa ke hadapan / opening balance), "income" = money received (收入 / 会费 / 乐捐 / 筹款 / pendapatan / kutipan / derma / yuran), "expense" = money paid out (支出 / perbelanjaan / bayaran / expenses), "closing" = the balance the page states at the end (结存 / 银行 / baki / baki bank / balance in hand or in bank), "other" = anything else (a budget, a target, a price quoted, a figure in a resolution). Only the lines of the TREASURER'S own report (财政报告 / Laporan Kewangan / Laporan Bendahari) take opening / income / expense / closing; an amount mentioned inside another report or item — a dinner's takings in the secretary's report, a scholarship handed out, a budget — is "other" even when it is money received or paid. One role per figure; when unsure, "other".
