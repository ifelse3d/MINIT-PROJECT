...
export function ConstitutionReadEstimate({
  ...
  return (
    <p className="rounded-sm border border-[color:var(--v2-border)] bg-white/60 px-3 py-2 text-sm text-muted-foreground dark:bg-white/5">
      🧮{" "}
      <Tri
        bm={`${e.pages} muka surat → 1 tindakan AI (pelan percuma: ${e.fencePages} muka surat daripada baki seumur hidup)${parts > 1 ? ` · dibaca dalam ${parts} bahagian` : ""} · anggaran ${timeBm}.`}
        zh={`这份共 ${e.pages} 页 → 扣 1 次 AI 用量（免费版终身页数扣 ${e.fencePages} 页）${parts > 1 ? `、分 ${parts} 段读` : ""}，预计${timeZh}。`}
        en={`${e.pages} page${e.pages === 1 ? "" : "s"} → 1 AI action (free plan: ${e.fencePages} of the lifetime page allowance)${parts > 1 ? ` · read in ${parts} parts` : ""} · about ${timeEn}.`}
      />
    </p>
  );
}
