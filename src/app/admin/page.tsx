...
async function loadRows(): Promise<OrgRow[]> {
  ...
  // ai_usage: org_id + cost + created_at + refunded_at. One bounded fetch —
  // the pilot fleet is small; when it is not, this becomes an RPC aggregate.
  const usage = await admin
    .from("ai_usage")
    .select("org_id, cost_micros, created_at, refunded_at")
