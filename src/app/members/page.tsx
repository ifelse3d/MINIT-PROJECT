...
export default async function MembersPage() {
  ...
  if (user && active) {
    ...
    // step (46 → 41 → 37 → 32 → base) — the page never white-screens over a
    // schema gap.
    // 130 §11: migration 46 adds the eROSES particulars; behind it, 41.
    const with46 = await supabase
      .from("committee_roster")
      .select(
        "id, position, person_name, name_official, term_start, note, honorific, email, state, phone, ic_no, address, occupation",
      )
      .eq("org_id", active.id)
      .order("id", { ascending: true });
    const with41 = with46.error
      ? await supabase
          .from("committee_roster")
          .select(
