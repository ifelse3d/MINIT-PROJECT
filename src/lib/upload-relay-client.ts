...
// the why. A PDF too big for Vercel's ~4.5MB body cap goes STRAIGHT to
...
// sent only the path.
//
// Every door calls maybeRelayLargePdf() right after the shrink+size check:
//   null        → not a relay case (not a PDF, or small enough) — the door
...
/**
 * Relay `file` via Storage when (and only when) it is a PDF that cannot ride
 * the request body. Files that are not PDFs, or fit the body cap, return null
 * and take their existing path.
 */
export async function maybeRelayLargePdf(file: File): Promise<RelayOutcome | null> {
  if (file.type !== "application/pdf") return null;
  if (file.size > RELAY_MAX_BYTES) {
    // The honest wall that remains: the AI vendor's own request ceiling.
    return { ok: false, error: joinUserError(USER_ERRORS.pdfTooBigForAi) };
  }
 ...
 * relay a big PDF via Storage, refuse honestly what neither road can carry.
 * Doors append `file` or `storagePath` to the form accordingly and show
 * `error` verbatim — no door invents its own limit logic again.
 */
export async function prepareUploadForSend(file: File): Promise<PreparedUpload> {
  ...
  // A-4: too big for the request body — a PDF takes the Storage relay.
  const relayed = await maybeRelayLargePdf(sent);
