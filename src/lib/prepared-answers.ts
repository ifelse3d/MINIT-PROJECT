...
export const SUGGESTED_QUESTIONS = [
  ...
  // 130 §12 (113 §沒做 3 / 119 A-11): two more that people actually ask —
  // both answered here for free (61 §1-6 ①; the test pins each language).
  {
    bm: "Nombor resit datang dari mana?",
    zh: "收据编号是怎么来的？",
    en: "How are receipt numbers made?",
  },
  { bm: "Apa itu eROSES?", zh: "eROSES 是什么？", en: "What is eROSES?" },
...
export const PREPARED_ANSWERS: PreparedAnswer[] = [
  ...
  {
    // 130 §12: a fixed-knowledge answer — the numbering is a RULE of the
    // product (Hard Rule 2: receipts are numbered by code, sequential and
    // gap-free per organisation, never editable), so the system can say it
    // without looking anything up. Listed BEFORE where_receipts on purpose:
    // "收据编号怎么来的" also contains 收据 + 怎么, and the more specific
    // entry must win.
    id: "receipt_numbering",
    route: "money_receipts",
    answer: {
      bm: "Nombor resit dijana oleh sistem — bukan oleh AI dan bukan ditaip: setiap pertubuhan ada satu siri, berturutan, tanpa lompatan, dan nombor yang sudah dikeluarkan tidak boleh diubah. Resit seterusnya sentiasa nombor terakhir + 1. Tekan butang di bawah untuk ke halaman resit.",
      zh: "收据编号由系统自动编 —— 不是 AI 编的、也不能手打：每个机构一条序列，按顺序、不跳号，开出去的号码不能改。下一张永远是上一张 + 1。按下面的按钮到开收据页。",
      en: "Receipt numbers are generated by the system — not by the AI and never typed in: one sequence per organisation, consecutive, gap-free, and an issued number can never be changed. The next receipt is always the last number + 1. Tap the button below to go to the receipts page.",
    },
    patterns: {
      bm: [{ any: [["nombor resit", "no resit", "no. resit", "penomboran resit"], ["mana", "macam mana", "bagaimana", "kenapa", "datang", "dikira", "dibuat"]] }],
      zh: [
        {
          any: [
            ["收据编号", "收據編號", "收据号码", "收據號碼", "收据的编号", "收據的編號"],
            ["怎么", "怎麼", "如何", "来的", "來的", "规则", "規則", "编的", "編的", "排的", "算的"],
          ],
        },
      ],
      en: [{ any: [["receipt number", "receipt numbers", "receipt numbering"], ["how", "where", "made", "generated", "come from", "work"]] }],
    },
  },
  {
    id: "where_receipts",
  ...
  {
    // K6 (82 §7): file intent in the chat = point at the door. 130 §9: the
    // chat HAS a door now — the 📎 paperclip under the box (one file at a
    // time, the same reader as the home page); the home page's box stays
    // the road for several photos at once and for long PDFs (the queue).
    // The answer says both; the button keeps pointing home (the fallback
    // for an environment where the paperclip does not work — 90's rule).
    id: "upload_file",
    route: "home",
    answer: {
      bm: "Boleh: tekan klip kertas 📎 di bawah kotak ini untuk melampirkan satu fail — gambar, PDF atau fail Office — dan MinitAI akan cam sendiri dokumen apa itu. Beberapa gambar sekali gus, atau PDF yang panjang, hantar dari kotak muat naik di halaman Utama (butang di bawah).",
      zh: "可以：按这个框下面的回纹针 📎 夹一个文件 —— 照片、PDF、Office 文件都行，MinitAI 会自己认出是什么文件。一次几张照片、或很长的 PDF，请从主页的上传框送（按下面的按钮）。",
      en: "Yes: tap the 📎 paperclip under this box to attach one file — a photo, a PDF or an Office file — and MinitAI works out what the document is by itself. Several photos at once, or a long PDF, go through the upload box on the Home page (button below).",
    },
    patterns: {
      bm: [
        {
