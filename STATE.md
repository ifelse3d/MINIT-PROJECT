...
**最后更新：2026-08-31 深夜（MYT）· Opus 5（105 号场：底座场——排队背景工＋会议记录两层＋侦测重复页）**
**🔴 本场（105 号场）状态一句话（107 号报告）：三件底座工都做了。
①**长文件排队慢慢读**——`ai_jobs` 表＋`/api/job/start`（只报价、一分不扣）
＋`/api/job/step`（一次只做【一批 4 页】，D47 逐批扣差额，失败只退自己那批
且 `pages_done` 不动）＋`/api/job/open`（同机构另一台装置也接得上）；
**关掉分页不算失败**，进度在资料库不在浏览器。②**会议记录多了「正式版」**
——对【已抽好的逐字 JSON】再跑一次纯文字整理（顺序／跨页合并／口语补句／
缺的标缺），🔴 `extract-meeting-notes` 一个 byte 都没动（`git diff src/prompts/`
只多一个新档），逐字层仍是唯一存档真相，正式版每一段一 tap 翻回原文；
锁死清单用**形状**守（整理 pass 根本拿不到金额/名册/日期）＋五道用数的检查
（coverage 少一条整份退／数字／中文串／拉丁人名要落在词界／决议语气），
过不了就退回逐字并标「照原文显示」。③**读完后自己发现重复页**会跳卡问，
门槛 0.30 是量出来的，而且**对照组救了它**（印刷 minit 的议程表行字面上
包含在段落里 → 加「两行长度要相当」才不乱问）。④**聊天框打字下指令真的会
照做**（J 原话「這兩張是一樣的…更詳細的」→ 当场出新成品卡，零重读零扣费，
补掉 103 §7 的残）。⑤104 挂的「排队慢慢读施工中」文案**拆掉了**。**
**测过：tsc 0 · eslint 20（基准逐字同）· vitest 1393（+65）· build ✓ ·
三条 e2e 全绿 · **新 e2e-105 全绿**（②聊天指令零重读、③两个分页与翻回原文；
①排队真跑 **SKIP——等 migration 43**）· shot-queue-105 8 项 PASS（报价卡／
进度条／成品卡／接着读黄卡＋两条「同画面只准一个百分比」）·
probe-slice-105 11 项 PASS（真 12 页 PDF 真切三批）· probe-duplicates-105 PASS ·
既有 8 支探针全 PASS · **会议记录 eval 39/41（95.1%）invented=0——与 8/30
那次 117/125 全套里的同 4 支逐案逐格一模一样** · 品质 eval 3/3 PASS 0 findings ·
全场真钱 **≈US$0.034**（授权 ≤1.20）。**
**🔴🔴 J 的三件事（107 号报告开头一分钟版）：①双击 `push-cabang.bat`
（本场 **9 支** commit）；②**这次一定要贴 migration 43**
（`20260921000000_ai_jobs.sql`）——**没贴＝排队整条休眠**，app 会安静地走回
旧路（长档案照旧读不完），不会坏、不会乱扣钱，贴完自动生效；③上线丢一份
12 页以上的 PDF 走一次，再开一份会议记录看「正式版」。
旧拍板照旧欠：bench 模型、eval 对外口径。**
...
> （104 场 10 支 J 已推上 ✓）；**105 号场 9 支 commit 等 J push-cabang.bat**。
...
1. **双击 push-cabang.bat（最要紧）**——105 号场 **9 支** commit 等推。
   ~~104 场 10 支~~ 已推 ✓。
