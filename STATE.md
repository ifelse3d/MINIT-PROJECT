...
**最后更新：2026-08-28 凌晨（MYT）· Fable 5（31 号单自动接力·场次 4：Stage E 品牌杠杆＋Stage F 小修包 F-1～F-11 全部＋Stage G-1/G-2 bench 工具链·mock 验证通过）**
**🔴 31 号单（`C:\dev\_J-要做的事\31-上線後修理總單-20260827.md`）夜里能做的场次 1～4 已全部勾完；剩场次 5 要 J 在场（十大观音照片→D-7/D-8、MyInvois 官方模板→B-7 对齐、J 双击 bench 实跑 G-3）。**
**给 J 看的这一轮报告：`C:\dev\_J-要做的事\25-大改造進度報告.md`（固定档名，已覆写，开头是「早上六步」）。**

---

## 🌙 现在在哪里（2026-08-28 凌晨，场次 4 收工）
...
> J 早上照 31 号单 §1 贴（salin-migration.bat 选 26）。场次 3、4 都没有开新 migration。
> **未 push 共 19 支 commit**（场次 1 七支＋场次 2 五支＋场次 3 三支＋本场 4 支）。本场：
> `22df599`（Stage E 品牌杠杆 brand.ts）· `1572fed`（Stage F 小修包 F-1~F-11）·
> `f1bfd5f`（Stage G-1/G-2 bench 工具链＋bat＋mock 验证）· 收工一支（勾单＋STATE＋25 报告）。

### 现场量到的（不是听说的）

- 四道关（Stage E、F、G 各收尾跑一轮）：`tsc` **0** · `eslint` **21（与基准逐字同：
  20 errors + 1 warning）** · `vitest` **798 全过（64 档；+8＝office-text 转档测试）** ·
  `build` ✓
- `e2e:money`：**15/15 全绿、page errors 0**（动了钱区入口、手动收入表单、聊天 route）。
- **E-1 的杠杆是真拉过的**：BRAND_NAME 改成抛弃名 → build → `.next/server` 里
  16 个产物跟着变 → 改回 "Minit" 再 build 干净才 commit。
- **F-3 四张图是新 UI 现拍的**：headless Chrome＋示范模式＋无 AI 打字路
  （scripts/howitworks-shots.mjs，测试 org 用完即删、零 AI 额度）；高亮框坐标
  是拍摄时对真实按钮量出来的百分比。修好后用无头浏览器把弹窗四格各截一张
  肉眼核对过（框正好落在「拍照」按钮/做好的文件上）。
- **G-1 mock 验证**：`npm run bench --mock`＝2 个假模型 × 10 个 golden case，
  0 失败案、0 真实厂商呼叫，报告落 `eval/reports/model-bench-<日期>-mock.md`
  （整夹 gitignore，属验证产物）。`--dry-run` 实测：候选收割出 nano/flash-lite/luna
  ＋haiku 因缺 key 列为跳过，预估总费用 ≈$0.11。
- **G-2 用的是真实记录**：ai_usage 里 3 笔 `chat_turn`（gpt-5.6-luna，平均
  input 1823 / output 131 tokens）→ 每题 nano $0.000144 vs luna $0.000522＝3.6×。
- ⚠ **没能验证的**：F-10 只在单元测试里喂过自己生成的 docx/xlsx（jszip/exceljs
  round-trip），没拿真实世界的 Word 档在线上流程走过一遍——J 手上随便一个
  .docx 丢首页框就是验收；bench 的**真跑**没跑过（拍板 45 禁止无人值守烧额度），
  等 J 双击；migration 26 照旧未套用、等 J。

### 这一场做了什么（细节都勾在 31 号单 §8 §9 §9b 各条目里）

- **Stage E（品牌杠杆，拍板 38/D20）**：`src/lib/brand.ts` 一行改名；接上
  layout metadata、侧栏＋手机 logo（含首字母方块）、首页、问一问标题、
  how-it-works 文案、全部文件的「Drafted by …」审计行（BM/zh/EN 共用
  draftedByLine 模板）＋PDF Producer＋minutes/actions 的审计行侦测。
  未接（故意）：句中文案 ~600 处、prompts（拍板 42）、legal（要人审）、注解。
  收据字首 MIN 与 brand.ts 无关联（查证过）。
- **Stage F（小修包）**：F-1 额度说法（方案页早期账号一行＋计数器新文案）·
  F-2 PDPA 一行＋详情展开＋IC 句改写 · F-3 how-it-works 四图新拍＋CSS 高亮框 ·
  F-4 对话存 scoped localStorage（首页 chat.home / 面板 chat.panel 各自 key）＋
  「清除对话」＋变慢提示 · F-5 chat prompt 教学型编号步骤 · F-6 ASK_ACTION_ROUTES
  四个动作深链＋?dari=ai＋FromAiNote 落地行 · F-7 「其他」备注必填 · F-8 示范
  徽章并一颗 · F-9 农历初一/十五献供开关（推导不落盘、无删除钮）· F-10 Word/Excel
  真转档（office-text.ts：docx=jszip（exceljs 既有传递依赖提为直接依赖，lock 仅
  +1 行）、xlsx=exceljs；/api/intake 收 .docx/.xlsx，先确定性抽文字（免 AI 额度）、
  40k 字上限拒绝不截断、文字以 untrusted block 挂在 extract prompt 后——prompt
  档一字未动；PPT 不做）。
- **Stage G-1/G-2（拍板 45）**：bench-models.ts 重写（候选自动收割自
  docs/换模型手册.md＋.env.example、剔除 embedding、只留有 key 的；跑前印预估
  费用＋等 Enter；排名表带每次读取成本＋CP 值＋机械性建议标签；报告
  `eval/reports/model-bench-<日期>.md`）＋`bench-models.bat`；mock 注入走
  eval/run-eval.ts 唯一新缝 `setEvalProviderOverride`（生产路径不会碰）；
  G-2 CHAT 段把 29 号单 §7 毛利口径问题印成两行数字，不替 J 选。
- 🔴 **没动的**：`src/prompts/extract-meeting-notes.ts` 一字未碰（D-7 等 J 照片，
  场次 5）；没开新 migration；Vercel/.env 值/git push/线上资料照旧没碰；
  bench 真跑一次都没跑（只 mock）。

### 🔴 J 的事（场次 4 结束时＝早上六步，写在 25 报告最上面）

1. **贴 migration 26**（salin-migration.bat 选 26 → SQL Editor → Run）。
2. `status.bat` 看 26 的两个探针 APPLIED。
3. **双击 push-cabang.bat**（本机领先 origin **19 支**）→ 线上自动更新 → /health 全绿。
4. 两个 Region 核对（Vercel Functions＋Supabase General）抄给 Claude。
5. 给材料：📷 十大观音手写原照（＋满意范本）· 📄 MyInvois 官方 Batch Upload 模板。
6. 开新对话贴 31 号单 §10 灰框 → **场次 5**（D-7/D-8/D-9、B-7 对齐、G-3 实跑、当日验收）。
   ⚠ **8/31 23:59 竞赛截止（内部 cutoff 18:00）**；one-pager 的 [YOU] 两处还空着。

### ❓ 未决问题

1. 🔴 Vercel/Supabase region 是否同区 —— 等 J 抄来两个值（不同区=每击多几百 ms）
2. 助手本身用哪个模型 —— G-2 已把 nano vs luna 每题成本印成数字（3.6×），
   决定等 J；真手写 bench（G-3）之后一起定
3. 法律实体（金流前置，D12），试点前要答
4. 真实手写 eval：92.9% 量的仍是印刷体。J 拍照后场次 5 建 golden case 重跑
5. Supabase 邮件模板（顺延：等网域＋SMTP）；Supabase Site URL 还停在 localhost
   （31 号单 §11，J 一分钟改）
6. （旧，小）打字格每行的转账不能附截图——真有人要再说
7. （旧，小）MyInvois 官方模板逐栏对齐（B-7 后半）等 J 给原档——D21 verify 警语在档上
8. （新，小）competition/screenshots 60 张是 8/26 拍的、对不上场次 1–4 的新 UI；
   one-pager 没直接引用截图档所以不挡提交；要重拍走 scripts/screenshots.mjs
9. （新，小）how-it-works 第 4 格是纯文字版式（正是 J 嫌不专业的样子）——
   场次 5 D-8 做完正式版式后重拍一次（scripts/howitworks-shots.mjs 一条命令）
10. （新，小）check-ai.bat 还 cd 到旧资料夹 C:\dev\minit——改天一行修

### ⏭ 下一个 session 从哪开始

**31 号单场次 5（早上，J 在场）：D-7 决议归类（J 照片进 golden case→改后处理→
npm run eval 对比过了才上）＋ D-8 正式报告版式（含 PDF；「该印什么」抽纯函数直测）
＋ D-9 四道关＋e2e:minutes ＋ B-7 后半（MyInvois 官方模板逐栏对齐）＋
G-3（J 双击 bench-models.bat 实跑，真手写口径写进记分表与这里）＋当日验收修尾。**
J 照 25 报告「早上六步」走即可，31 号单上已勾 ✅ 的不重做。
...
- ⚠️ **正则从散文里收割「provider:model」清单，会把句号吃进模型名、把 embedding
  ...
  （尾巴带句点）和三个 embedding 模型（它们不会读取、只会白烧钱）。**修法：
  正则尾锚定字母数字（`[A-Za-z0-9]$`）＋按用途黑名单过滤；判断方法：任何
  「从文件里 grep 清单」的工具，先 --dry-run 把收割结果印出来人看一遍。**
- ⚠️ **查询线上表之前先确认枚举值的真实拼法。** G-2 第一版按直觉查
  `action=eq.chat`，而 ai_usage 里实际是 `chat_turn`——查询成功、返回 0 行、
  代码「诚实」地退回假设值，看起来像没有数据，其实是问错了问题。**判断方法：
  对着真表先跑一次 `select action, count(*) group by`（或等价 REST 探针），
  再写过滤条件。**
- ⚠️ **给 heredoc 里的 Python 传中文长文案，内嵌英文双引号会当场炸语法。**
  勾单脚本里「改回 "Minit"」把 Python 字符串截断。**修法：长中文＋引号混排的
  批量编辑，用 Write 工具写成 .py 档再跑，别走 bash heredoc。**
- 💡 **示范模式＋无 AI 打字路＝零额度的截图素材生产线。** F-3 四格全靠它：
  示范模式给「读出来待核对」的画面，打字路（自己打字→填三格→做好的记录）给
  「真的做好的文件」，全程不碰厂商。以后要拍流程图先想这两条路。
...
- ⚠️ **无人值守的 session 里 Browser 面板不显示＝截图必挂（页面不合成画面）。**
  ...
  （场次 4 补充：headless Chrome 的 `page.screenshot()` 不受此限，F-3 的四张图
  和验收截图都是这么拍的。）
...
- 🔴 **同一个 localStorage key，两个 `usePersistentState` 不会同步 —— 而且是无声的。** 每个实例各有一份 React state：两个组件同时挂载，各自在 mount 时读一次 key，然后**再也听不到对方写了什么**。第二个组件的写回 effect 会拿它那份过期的清单盖掉第一个的编辑。拆 `/money` 的时候最顺手的做法就是每页各自 `usePersistentState`，而那会让**钱无声无息地变回去**。正解是 **layout 里一个 Provider 拥有那份状态**（`src/app/money/register-store.tsx`）。<br>**判断方法**：任何「把一页拆成好几页」的工作，第一个问题是「这份状态谁拥有」，不是「JSX 怎么搬」。（场次 4 的 F-4 同款教训：首页与浮动面板的对话**故意用两个不同的 key**。）
- 🔴 **程式一定会有一段时间比资料库新 —— 而 PostgREST 只要被问到一个不存在的栏位，整个 query 就失败。** J 是自己手动跑 migration 的（D8），所以「写好写入端」和「资料库真的有那些栏位」中间永远有一段时间差。这一段时间里，一个 `select` 多要了一个还不存在的栏位，**整页会变空白** —— 而空白的行事历跟「这个社团没有活动」长得一模一样。<br>**做法**：每一支 action 都回传结果物件、**绝不 throw**，把「存不进去」当成正常结局由 UI 说一句话。
- 🔴 **「写进资料库」不是一个答案，是三个不同的答案。** 同一批工作里，`events_meetings` 和 `remittance_batches` 真的需要写；`deadlines` **不需要** —— 死线是**算出来的**，两个输入都已经在资料库里。把算出来的东西再存一份，是**第二份可能跟第一份不一致的资料**，比不存更糟。真正缺的是另一半：`status = 'done'` 的打勾。<br>**判断方法**：先问「这份资料是记录的，还是推导的」。推导的东西同步的是**人的决定**（打勾），不是数字本身。（场次 4 的 F-9 同款：农历献供日**推导、不落盘**。）
...
- 🔴 **价目表会腐坏，而且是「促销到期」这种有日期的腐坏。** 每次加模型进价目表，连「查证日期」和「促销到期日」一起写。（场次 4 起 `PRICES_CHECKED_ON` 从 unit-economics 导出，bench 印价必带此日期。）
...
- ⚠️ **动 prompt 之前先问「这让哪些量测作废了」。** 词库为空时 prompt 逐字不变，并有测试守着。
...
- 🔴 **批次汇入宁可整批拒绝，不要只加一半。**
- 🔴 **给了上传按钮，就要吃使用者手上真的有的档案。**（场次 4 的 F-10 就是这条的续集：.docx/.xlsx 终于真的吃了。）
- 🔴 **能用程式解析的，不要送去 AI。** 画面上写明「这是程式读的，不会用掉您的额度」。
- ⚠️ **试算表里的空格是「位置」，不是「没有」。**
- 🔴 **「哪一条路会花钱」要写在按钮上，不是写在说明里。**
- 🔴 **AI 读出来的东西不要直接进资料库。** AI 出稿、人确认才写。
- 🔴 **`name_official`（IC 上的名字）不是翻译，不可以放进词库、更不可以让模型生成。** 独立栏位、人手照 IC 抄。
...
- **被 gitignore 的东西，在 repo 里等于不存在。**（bench 的 mock 报告在 eval/reports/ 里，属验证产物，不入库。）
- **`.gitignore` 挡不住已经在追踪中的档案。** 要 `git rm --cached`。

### 关于 git 与环境

- **在沙盒／非互动环境不要跑任何会等待输入的指令。**（场次 4 的 bench 为此专设 `--mock` 自动继续、`--yes` 跳确认；给 J 的 bat 才走「等 Enter」。）
- **「已 commit」不等于「已 push」。** 两个动作分开写。（本轮：**全部已 commit、全部未 push，共 19 支。**）
...
- 🔴 **PowerShell 批量替换源码档：读和写都要明示 UTF-8**（见 8/27 那条——8/25 的教训只做一半照样炸）。
- **沙盒连不到 Supabase，也下载不了二进位档。** migration 套到第几支、eval 跑出什么，Claude 只能读报告。（本机 Claude Code 反而连得到——e2e、截图脚本、G-2 的 ai_usage 查询都是本机直连跑的。）
- **沙盒不要 `npm install` 进 `node_modules`。**（本机 EACCES 时：`node node_modules/typescript/lib/tsc.js`、`node node_modules/tsx/dist/cli.mjs`、`node node_modules/npm/bin/npm-cli.js install --package-lock-only` 全是能用的绕法——场次 4 实测。）
...
- **npm 漏洞剩 2 个（exceljs/uuid）刻意不修**：`audit fix --force` 会把依赖降破。等上游。（场次 4 新增直接依赖 jszip 3.10.1——本来就在 exceljs 依赖树里，零新安装。）
- **「从来没有程式写入过的表」清单已大幅缩短**：剩 `extractions`/`einvois_packs`/`paste_packs`/`qa_log`/`reminders`/`rsvps` 仍无写入端。
...
- **`office_bearers` 是法定申报栏位。** 放东西进去之前先看 `src/prompts/eroses-map.ts`。
...
（更省事：双击 `salin-migration.bat`，选 26。）
...
| `docs/` | `DECISIONS.md`（D1–**D21**）· `功能盤點-計劃vs實作.md` · `产品缺口盘点.md` · `上线与截图-给J的步骤.md` · `换模型手册.md` · `AI-API-选型与成本.md` · 其余照旧 |
| 定价／毛利 | `src/lib/unit-economics.ts` + `npm run economics`。讲毛利以这里为准（价目表查证日 `PRICES_CHECKED_ON`） |
| AI 分流设定 | `.env.example` 的 AI 段 + `npm run check:ai` |
| 模型对比 | `npm run bench`（--dry-run / --mock）· 给 J 的 `bench-models.bat` · 报告在 `eval/reports/model-bench-<日期>.md` |
...
| 给 J 双击的 `.bat` | `status.bat` · `salin-migration.bat` · `salin-env-vercel.bat` · `push-cabang.bat` · `bench-models.bat`。🔴 `push-to-github.bat` 不能用（会 `git add -A` 吞垃圾）；⚠ `check-ai.bat` 还指旧资料夹 |
| `competition/` | 顶层＝当前版（**[YOU] 两处还空着**）；`screenshots/` 60 张 8/26 拍、已对不上场次 1–4 新 UI（不挡提交，要重拍走 scripts/screenshots.mjs） |
