...
**🟣 136 急修二（2026-09-09 ~5:00 PM，J 拍板「一律不印 dengan maaf」，推翻 134 那条）：** 纸上写「请假」或「缺席」，文件都只印 `tidak hadir: N orang`、标题只印 `TIDAK HADIR`；`bm-glossary` 请假→`Tidak hadir`；人数卡同。`parseHeadcount.excused` 照旧解析、只是不印。改 `minutes-lang.ts`／`headcount.ts`／`bm-glossary.ts`／`headcount-card.tsx`＋测试。

**🟣 136 急修（2026-09-09 ~4:50 PM，J 线上抓：「AGENT 不会看支出还是收入，全部变成收入」，`main`＝`db061bd` 已 push）：** 画面上七行是**下午 4 点那次读取**（存在浏览器 localStorage，旧 prompt 读的、本来没有 `kind`），新版 UI 撈出来显示成「Row type?」——**不是模型不会分**：用同结构合成页打线上同一颗 flash-lite，7/7 全对（`scripts/probe-136-ledger-raw.ts` 印 zod 前的原始 `kind`）。修两处：①`rowKindOf` 在读取端没标时也用**支出词表**（礼堂／租金／开销／sewa／dewan／kos…）与**收入词表**（会费／乐捐／捐／yuran／derma…）推灰色「码推的」徽章，**只在单边命中才推**（「Derma pembaikan bumbung dewan」两边都有→不猜）；码推的支出永不进收据，码推的收入等同无标（其余字段照旧要人确认）；模型自己的标签永远优先。②整份读取没一行有标 → 琥珀提示「这份是 MinitAI 学会分收入／支出之前读的……重新选照片按『重拍这一页』」（`data-probe="ledger-stale-kinds"`）。vitest 1659（+3）· tsc 0 · eslint 0。🔴 陷阱：python 普通字串里的 `` 变退格字元把正则弄哑——正则一律 raw string／Write 整档。

**🟣 136 工作场（2026-09-09 傍晚；**~5:05 PM J 说「直接 PUSH，好了我直接检查」→ `main` 已 fast-forward 到 `f6751e7`（＝after-stage，130／135／136 全部）并 push，线上换版后即是这份**；merge 时 `salin-migration.bat` 假差异挡住 `--ff-only`，改在 main 上 `git reset --hard after-stage`（等价 ff）；报告在 `_J-要做的事\138-報告-136場-Money分類-demo就緒-20260909.md`）：**J 拍板三题（135 §B）全落地——读取端每一行标 `kind`（income／expense／balance／total）。①契约 `donationRowExtractionSchema.kind`（`.optional().catch(undefined)`，旧资料照过、乱值丢标签不丢行、`missing` 在 parse 时删掉＝没标；**码永远不补成 income**）；`src/lib/ledger-hints.ts` `rowKindOf`（模型标 → 135 结存词表当备援 → null）、`kindAllowsReceipt`；`eligibleForReceipt` 只放 **confirmed income**（或旧资料无标且不像结存），`check` 不算。②`extract-ledger.ts` 加 kind 规则：总计行**改成输出标 total**（码藏起来），分不清→check＋snippet 写为什么、**不准预设收入**，整页无捐款人＝财务总结逐行输出不造 donor；Hard Rule 2 原句照抄；`extract-meeting-notes.ts` 一字没动。③UI `kind-badge.tsx`：徽章颜色跟 confidence（check ❓＋一键「对」）、点徽章换收入／支出／结余（人改＝confirmed、`userSource()`）、码推的灰色「码推的」、没标不显示徽章只给「这行是什么？」；支出行一键「记成支出 → 支出与报销」（`expense-handoff.ts` 新 sessionStorage 欄位 parcel，`expenses-view` 进页预填金额／用途／日期／缩图，**人按存才写**；该行标「已送去支出」、徽章冻结）；balance／total 灰字「不是收到的钱，不会开收据」；total 默认收起「已隐藏 N 行总计（显示）」；135 黄卡加一句。④eval：`ExpectedLedger.rows[].kind?`（enum 比对），case-05/06/07 全补 income，新 `case-11-ledger-agm-summary`（PIL 合成印刷 KEWANGAN 页，全虚构）。**一轮就过关：176/184 · 95.7% · 捏造 0 · 舊 10 案每格与 9/7 报告逐格相同（127/135，多的 10 个 kind 全对）· case-11 49/49、kind 8/8，US$0.045**。⑤`e2e:money` 加 17 项（parcel 手塞、只算 `offsetParent` 可见）37/37；新 `scripts/probe-136-money-real.mjs` 真读两张（case-05 首页一门→/money 4 行 income→加入登记→MIN-2026-0001→PDF 200；case-11→黄卡＋7 徽章＋Jumlah 收起＋登记 (0)→一键支出→预填 RM1,000「Sewa dewan 礼堂租金」），截图在 `eval/reports/136-real-*.png`。四关：tsc 0 · eslint 0/0 · vitest **1656**（+16）· build（见 138）。真钱本场 **≈US$0.10**（eval 0.045＋真读 3 次 ≈0.05；授权 ≤1.00）。**「上台完了」的 merge 已做掉，之后 J 说上台完了只需 fetch 确认，不要再 merge。**本场发现：`extraction-versions.ts` 根本没有 ledger 合并（施工单写错），`mergeLedgerExtractions` 原样 spread 行所以 kind 自然跟着走（有测）；preview_start 在这台机器 EACCES，dev 用 Bash 背景起；`page.click()` 会点到 ExtractionTable 手机版隐藏副本→e2e 一律 `clickVisible`。**

**🟣 135（2026-09-09 下午 ~4:15 PM，只在 `after-stage`，上台后随 merge 上线；报告＋施工单在 `_J-要做的事]-施工單＋報告-Money頁五件-英文預覽-20260909.md`）：**J 把 AGM 手写记录拍进「Record income」，读出七行没有捐款人的「捐款」（上年结存、会费、乐捐、晚宴、礼堂、晚晚餐宴、银行）——①从首页一门进来的账页看不回原图→`register-store` 交接块把 parcel 的 `photoDataUrl`／`storagePath`／`pages` 装进 `ledgerPages`，`ledger-review` 的 PageThumbs 接 `signedUrlForOriginal`；②审核行删不掉→`removeLedgerRow`（`src/lib/ledger-hints.ts` `dropLedgerRow` 纯重编号，`addedRows`／`ledgerPayments` 跟着行走）＋每行 ✕；③「晚晚餐宴」→ 用 129 D 的 `struckDoubleSuggestion` 给一键「用『晚餐宴』」；④整页没有捐款人＝财务总结不是账页→`looksLikeSummaryPage` 黄卡（指向逐笔账页／打字／支出页）；结存／银行字样→`looksLikeBalanceRow` 行内提示「这是结余不是收入」，只提示不删；⑤（134 补）英文纸的预览用英文：`sourceLanguageOf` 用虚词计数分 en／bm。**AI 自己分收入／支出／结余（row `kind`）没做**——要改 `extract-ledger` 规格＋eval，135 §B 写了规格等 J 拍。验证：tsc 0 · eslint 0 · vitest 1640（+10）· 本机拋弃式帐号把手做的 parcel 写进 sessionStorage 走 /money：缩图／黄卡／两个结余提示／一键修正／删行 10/10。

**🟣 134 号线上急修二（2026-09-09 下午 ~3:50 PM，上台前 1.5 小时，`main` `0df3012` 已 push；报告在 `_J-要做的事\-報告-線上急修二-章程摺疊-缺席-預覽語言-20260909.md`）：**J 用 demo 帐号真跑一遍后抓三件——①章程页有「(3) lanjutan Fasal 10」「Fasal 4(3)」「(4) lanjutan」等零散卡（分段读的条尾被当成独立条、按前导数字排序）→ 新 `src/lib/constitution-fold.ts` `foldContinuationClauses`：条尾折回母条（指名 Fasal N 或前一条；句中断句接回同一句），flatten 层（新读存折好）＋ load 层（`loadConstitutionClauses`／`org-tools` 旧书读出即折，存的字节不动）；②纸上写「缺席」文件却印「TIDAK HADIR (DENGAN MAAF)」→ `parseHeadcount` 记 `excused`（请假/apolog/maaf/cuti 才算）、`absenceIsExcused()` 看人数行或请假名的 snippet，标题／Kehadiran 行／人数卡／免费预览都跟纸走（`TIDAK HADIR`／`缺席`／`ABSENT`）；③预览稿一半华语一半 BM → `sourceLanguageOf()`（数字符，零 AI）判纸的语言，store 里草稿阶段用纸的语言渲染（zh 水印、zh 标签、不套词汇表、不印「翻译本」注），全部确认后才用文件语言（BM）——同一版式。`renderMinutesDraftBm` 多 `lang`／`sourceCopy`；Labels 加 `absent`／`discussedAndDecided`。验证：tsc 0 · eslint 0（main 上那个 set-state 旧洞不算）· vitest main 1601／after-stage 1630（+20）· 拋弃式帐号线上种一本带条尾的书看 /constitution 折好（见报告）。**没能在线上看华文预览**（要真读一张华文纸＝花钱）。`after-stage` 已 merge main（`9117894`，三档冲突取 after-stage 版本），ff-only 照旧可用。
