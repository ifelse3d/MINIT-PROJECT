# Minit — 交接与设计决策
**日期：2026-07-29**
**用途：换电脑、换 Claude 帐号之后，用这一份文件接上进度**

---

## 0. 新对话第一句话该说什么

到新电脑、开新 Claude 帐号之后，第一句话就贴这段：

> 读 `CLAUDE.md` 和 `docs/DECISIONS.md`，然后从「下一步该做什么」那一节的第一项开始。不要跳步，动手前先跟我确认计划。

这样就不用重新讨论一遍。

---

## 1. 换电脑完整步骤（写给完全不懂的人）

### 在旧电脑（现在这台）做的事

**第 1 步：把工作存到 GitHub**

双击 `push-to-github.bat`

看到 **「SIAP / DONE」** 才算成功。看到 **「GAGAL / PUSH FAILED」** 就照窗口里的指示做，**不要关掉、不要删任何东西**。

**第 2 步：把钥匙文件抄出来**

有一个文件叫 `.env.local`（在 `C:\dev\minit` 里面）。它装着 AI 和资料库的密码，**故意不进 GitHub**（这是对的，密码不能上传到网上）。

所以要手动带走。安全的方法：

- 用 USB 随身碟拷贝，或者
- 存进密码管理器（1Password、Bitwarden 之类），或者
- **最安全：不要带，到新电脑重新申请一次金钥**（Gemini 的金钥在 Google AI Studio 可以随时重新产生；Supabase 的金钥在 Supabase Dashboard → Project Settings → API 看得到）

⚠️ **绝对不要**把 `.env.local` 的内容贴进聊天、email、WhatsApp 或任何云端笔记。

**第 3 步：记住这件事会发生**

新电脑打开 Minit 的时候，**捐款登记册会是空的**，但收据还在。这不是坏掉了。原因看第 4 节的 P1-2：有一部分资料目前只存在浏览器里，不在云端。这正是要修的问题之一。

---

### 在新电脑做的事

**第 1 步：装三样东西**（各自去官网下载，一路按「下一步」）

1. **Git** — https://git-scm.com/download/win
2. **Node.js** — https://nodejs.org （选 LTS 那个）
3. **VS Code**（选用，看程式码方便）— https://code.visualstudio.com

**第 2 步：把专案抓下来**

按 `Win` 键，打「cmd」，开启「命令提示字元」，然后一行一行贴进去、每行按 Enter：

```
mkdir C:\dev
cd C:\dev
git clone https://github.com/ifelse3d/minit.git
cd minit
npm install
```

第一次 `git clone` 会跳出窗口叫你登入 GitHub，登入就好。

**第 3 步：放回钥匙文件**

把 `.env.local` 放进 `C:\dev\minit\`（第 2 步抄出来的那个），或者按 `.env.example` 的格式重新填一份新申请的金钥。

**第 4 步：确认没问题**

双击 `Buka Minit.bat` → 浏览器会自己打开 `http://localhost:3000`

打不开的话，去 `http://localhost:3000/health`，那一页会告诉你缺什么。

**第 5 步：以后每天的习惯**

| 时机 | 做什么 |
|---|---|
| 开工前 | 双击 `pull-from-github.bat` |
| 收工前 | 双击 `push-to-github.bat` |

就这两个。`pull` 那个如果发现你有还没存的东西，它会停下来叫你先存，**不会覆盖你的工作**。

---

### 两台电脑都能改吗？

**能，但不要同时改。**

- **资料库（Supabase）在云端** → 两台看到同一份资料，完全不用担心
- **程式码要靠纪律** → 一次只让一台是「活的」

规则很简单：**离开一台之前 push，到另一台开工之前 pull。**

忘了 pull 就开始改 → 会出现 git 冲突，对新手很难处理。所以宁愿多按一次 `pull-from-github.bat`，它很快，而且它会保护你。

---

## 2. 这次讨论定下来的决定

以下八条是 2026-07-28 ~ 29 讨论的结论。**动手前先读这一节，不要重新发明。**

### D1 — 注册的是「人」，不是「组织」

**决定**：使用者注册 ≠ 自动得到一个组织。

**流程**：

1. 注册完，先给一个**示范组织**（`orgs.is_demo` 这个栏位已经存在，目前只是个标签、没有行为）。可以立刻用假资料试完整流程，不用等人批准、不用先付钱。**真实组织数 = 0。**
2. 要用真实资料 → 「建立我的组织」，必须填**社团名称 + PPM 注册号码**
3. 系统用 PPM 号码比对：**已经有人建过 → 不给建，改成「申请加入」**，通知该组织的 `hq_admin` 批准

**为什么**（这是最重要的理由，不是防刷）：

同一间庙的秘书和财政各自注册 → 各自自动得到一个组织 → **变成两个 org、两本登记册、两套收据号码**。收据号码是连号不可重复的法定凭证，一分裂就毁了，而且很难合回去。这比有人多开几个 org 蹭免费额度严重得多。

用 PPM 号码当唯一键在这个市场特别合理 —— 客户本来就是**有政府注册号码的社团**，这是现成的唯一识别，一般 SaaS 没有这种东西可用。

### D2 — 额度对外显示 %，对内记 token 和成本

**对使用者**：显示百分比进度条（像 Claude、GPT 那样），不显示次数。

**理由**：可以换模型、换供应商、优化 prompt 而**使用者感受不变**。用次数计价的话，成本结构一改就得重新谈。而且大档案自然贵、小档案自然便宜，不用另立规则。

**代价**：「剩 40%」不能告诉庙的财政够不够用到月底。所以要**两层显示**：

- 卖方案的时候用工作量描述：「每月约 20 份会议记录 + 300 张收据」
- 系统里显示 %，旁边配白话估算：「剩 40% · 大约还能处理 6 份会议记录」

**对内**：`ai_usage` 表要加栏位 —— `input_tokens`、`output_tokens`、`model`、`provider`、`cost_micros`。

⚠️ `cost_micros`（成本，整数微分单位）**必须在写入当时就算好存下来**。不同供应商 token 单价不同、而且会调价；只记 token 的话，换一次供应商所有历史数字就失去可比性，毛利算不出来。

### D3 — 大档案：现阶段只做提醒和硬上限，**不做百分比预测**

现在的漏洞：**一张手写笔记照片和一份 60 页章程 PDF，收费完全一样（2 次），但 token 可能差 30 倍以上。** 这不是「可能亏」，是确定亏。

各项难度差很多：

| 要做的事 | 难度 | 准不准 |
|---|---|---|
| 数 PDF 页数、图片张数、位元组数 | **很简单**，不用呼叫 AI | **完全准确**（数出来的） |
| 从页数推 input token | 中等 | 还可以（图片按尺寸切格计价，其实固定） |
| 预测 output token | **难** | **不准** |
| 给 `max_tokens` 封顶 | 很简单 | 最坏情况变已知 |

**所以现阶段只做四件**：

1. 数页数/张数 → 超过上限直接拒绝，建议拆开处理（目前只有 `MAX_BYTES = 8MB` 一个门槛，`src/app/api/intake/route.ts`）
2. 大档案给**白话提醒**，不报数字：「这份 32 页，比一般会议记录大很多，会用掉比较多额度」
3. 每次呼叫都设 `max_tokens`（输出 token 通常比输入贵 3–5 倍，生成失控比读长 PDF 更贵）
4. 长章程改成**分段处理**，一段一段扣，中途可以停

**顺序是：先记录 → 再观察 → 才预测。** 跑一两个月拿到真实 token 资料之后，再做校准过的预测。现在硬做预测一定不准，而**不准的预测比没有预测更糟**（使用者会不信任整个额度系统）。

### D4 — 免费策略：绑帐号的一次性小额试用

- 免费额度给**帐号**，不给 org（否则开一堆 org 就刷爆）
- **一次性**，不是每月给
- 额度给小 —— 够拍一次会议记录 + 开一次收据，看到价值就够
- 注册要验证（手机 或 PPM 号码）
- **免费层不含分会、不含额外成员席位**

⚠️ **收费前的硬门槛**：目前用 Gemini **免费层**。`CLAUDE.md` 明文写着免费层可能拿输入去训练，PDPA 上**只能放假资料**。所以在切到付费 API 之前，不能让任何真实社团上传真实捐款人姓名和身份证号。这不是技术细节，是商业前提。

### D5 — 方案分层

| | 免费试用 | 基本方案 | 分会方案 |
|---|---|---|---|
| 示范组织 | 1 | 1 | 1 |
| 真实 root 组织 | **0**（待确认，见第 5 节） | 1 | 1 |
| 额外 root 组织 | 0 | 0 | 0（另购） |
| 分会 | **0** | **0** | 依方案 N 个 |
| 成员席位 | 1（自己） | 依方案 | 依方案 |
| AI 额度 | 一次性小额 | 每月 % | 每月 %（较高） |

**技术现况**：`parent_org_id` 树状结构、`org_descendants()` 递回查询、HQ 角色自动继承到分会、删除时挡住还有分会的 org —— **全部已经做好了**。缺的只有「这个帐号的方案允不允许建分会」这一个检查。

⚠️ 现在任何人都能建分会。**要先关上**，不然最贵的功能在免费层就送出去了。

### D6 — 额度余额改成只增不改的流水帐

`orgs.extra_credits` 现在是一个可以 UPDATE 的整数 → 被改了查不出是谁、什么时候、改多少。

**改成 ledger（流水帐）**：每一笔加值、消耗、退款各一列，**只增不改**，余额 = 全部加总。

这正是 `CLAUDE.md` 规则 2 对收据的要求（连号、不可改、无断号）。**额度就是钱，套一样的规则。**

### D7 — 管理后台先做「错误与失败追踪」

现在**完全没有**：没有 superadmin 角色、没有 Sentry、没有 analytics，整个 `src/` 只有一个 `console.` 而且在注解里。要给某个社团加额度、批 s.44(6)、加成员，都得手动进 Supabase 跑 SQL。

**先做的**：错误与失败追踪 —— 哪个 org 的哪个功能失败了、AI 回传解析失败、上传失败。

**关键做法（PDPA）**：**记事件，不记内容。** 记「org 12 的 `extract_ledger` 在 zod 验证失败」，**不记**那张照片里有什么。这样既能除错，又不违反规则 5。

**后面才做**：注册与用量总览、手动操作工具（改额度、批税务状态、加成员、停用帐号）、异常侦测（一个帐号连开 5 个 org、某个 org 用量突然 10 倍、同一 IP 注册 20 个帐号）。

### D8 — Migration 用 CLI 管理，schema 先、程式码后

**现况**：`supabase/migrations/` 有 5 个档案，但**没有任何套用纪录**，全靠手贴 SQL —— 所以有两个漏掉了没人发现（见 P0-1、P0-2）。

**Claude 无法连线到 Supabase**（沙盒对外网路是白名单制，实测回传 `000`）。所以套 migration 一定要**你在自己电脑上执行**。两条路：

- **短期**：Claude 写好 SQL，你贴进 Supabase Dashboard → SQL Editor
- **之后**：装 Supabase CLI，Claude 写 migration 档、你跑 `supabase db push`（它会记住哪些套过了）

**铁律**：

> **资料库 migration 必须在程式码部署之前套用。**

现在 `src/app/money/actions.ts` 拒绝写 `client_id`，就是因为程式码先上了、migration 没套。规则：先套 schema、再部署程式码，而且程式码要能容忍「栏位还没有」的状态。

---

## 3. 安全五条规则

### 规则 1：钱和权限的栏位必须用 trigger 锁死，只有 service role 能动

包括 `extra_credits`、`monthly_free_quota`、`tax_exempt_status`、`parent_org_id`、`role`。

### 规则 2：凡是浏览器可写的栏位，都当成敌意输入

这是 `extra_credits` 漏洞的**真正原因**，不是谁写错一行：

Supabase 的设计是「浏览器拿 anon key 直接打 PostgREST，靠 RLS 挡」。RLS 能挡「哪些**列**」，但**挡不了「哪些栏位」** —— 所以只要允许 `hq_admin` 更新 `orgs`，他就能更新 `orgs` 的**任何**栏位，包括自己的额度。

**UI 上把输入框拿掉完全没用**，devtools 里一行 fetch 就绕过。必须在资料库层挡。

大平台的做法是所有写入都经过自己的 API 层、资料库从不对外。不需要整个改架构，但要记住这条规则。

### 规则 3：配额之外还要加速率限制

**这是两件不同的事**：配额防超支，速率限制防滥用。

现在**没有任何速率限制** —— 有人可以在 3 秒内把 100 次额度打完，或者拿这个服务当免费 AI proxy 把 Gemini 帐单打爆。

Claude 那边是「5 小时 session 额度 + 每周额度」两层叠，就是这个用途。

**加的位置**：`src/proxy.ts`（已经是每个请求都经过的地方）。

### 规则 4：加 idempotency key，防重复扣费

网路不稳时使用者按两下 → 扣两次。收据那边有 `client_id` 已经做了一半（但那个 migration 也没套用），**AI 呼叫这边完全没有**。

### 规则 5：除了 superadmin，所有角色都不可信

现在的模型隐含假设 `hq_admin` 是好人（所以让他改自己的额度）。任何多租户系统的铁律是：**除了自己的 superadmin，其他所有角色都不可信。**

不是说社团的秘书会作恶，而是**「他做不到」比「他不会做」可靠得多**。

**已经做对的一件事**：`checkAndRecordUsage` 在呼叫供应商**之前**扣费，这是正确顺序。反过来（先服务再扣）会被人用断线的方式白吃。

---

## 4. 已知问题清单（按优先顺序）

### P0 — 安全与金钱，最先修

**P0-1：`extra_credits` 自助充值漏洞** 🔴
任何 `hq_admin` 都能改自己的 AI 额度余额。`src/app/orgs/credit-form.tsx` 有输入框和「保存」，但**就算把 UI 拿掉也没用**，浏览器可以直接 `PATCH /rest/v1/orgs` 改成 999999。

程式码里的注解自己都承认了：
> `// This is a SELF-SERVICE AI-quota grant: the organisation's own admin sets their own credit balance. That defeats the metering layer`

**修法**：套用 `supabase/migrations/20260728000000_lock_org_privileged_columns.sql`（**已写好，标头写着 NOT YET APPLIED**）+ 把 `credit-form.tsx` 改成纯显示。

⚠️ **套之前要先验证**：`refundUsage()`（被礼貌拒绝时把额度加回去）会不会被 trigger 挡掉。它走 service role、理论上有豁免，但之前的审计报告提过这里有 bug。**「拒绝不扣额度」是 `CLAUDE.md` 规则 10 的硬要求，不能弄坏。**

**P0-2：收据号码目前改得动** 🔴
`supabase/migrations/20260726000000_client_id_and_receipt_lock.sql` 也**没套用**，所以收据身分不可修改的 trigger 没生效。对财务凭证是致命的。同时 `donations.client_id` / `source` 栏位不存在，`src/app/money/actions.ts:124-127` 因此拒绝写入。

**P0-3：建立组织没有任何数量上限** 🟠
`src/app/orgs/actions.ts` 的 `createOrg` 没有 count 检查，而每个新 org 自动得到 `monthly_free_quota = 100`（DB 栏位预设值）。开 10 个 org = 每月 1000 次免费 AI。→ 按 D1 + D5 修。

**P0-4：没有速率限制** 🟠
见安全规则 3。

**P0-5：`parent_org_id` 可被自助修改 + 递回查询没有防迴圈** 🔴（2026-07-29 白天发现，已修在 migration 里，尚未套用）
安全规则 1 列了 `parent_org_id`，但 `20260728` 那个 migration 原本只锁三栏、漏了它。
`orgs_update` 这条 RLS 的 `with check` 只验「这列是不是你管的」——改上层时 id 不变，**检查会过**，所以任何 hq_admin 都能从浏览器 `PATCH` 改自己的上层。
真正致命的是第二半：`org_descendants()`（`20260719000000`）是没有 cycle 侦测的 `with recursive ... union all`，而 `accessible_orgs()` / `_writable()` / `_admin()` 全部呼叫它，**等於每一条 RLS 政策都呼叫它**。所以一个「自己是自己的上层」就会让资料库每一次查询无限递回 —— 全站停摆，而且停摆后 UI 读不到任何东西，只能进资料库手救。可以被恶意触发，也可以被误触。
**修法（已写进档案）**：trigger 加锁 `parent_org_id`；`org_descendants()` 换成带路径、拒绝重访节点的版本。两边都要做 —— 锁挡住浏览器，路径版挡住其他管道（service role、逃生门、以后的管理后台、手打的 UPDATE）。
成本零：`src/` 从来没有 UPDATE 过这一栏，trigger 只管 UPDATE，建分会不受影响。

### P1 — 产品目前还不能被团队使用

**P1-1：没有「邀请成员」的功能** 🔴
`members_roles` 表和 RLS 政策**都写好了**（六个角色：`hq_admin` / `committee` / `secretary` / `treasurer` / `collector` / `auditor_readonly`），但**没有任何页面或 API 能新增成员**。唯一的写入是 `createOrg` 帮建立者自己插一列 `hq_admin`。

`/orgs` 页面的空状态甚至写着「叫管理你们组织的人把你加进来」—— 而那个人**没有任何按钮可以按**。

后果：一个社团的秘书和财政**无法在同一个 org 里协作**。这不只是定价问题 —— 「多少钱开给多少人」这个方案现在没有东西可以卖。

（D1 的「申请加入」流程会顺便解决一半：成员可以自己申请，不用纯靠 admin 主动邀请。）

**P1-2：一半的资料只存在浏览器里，不跨装置** 🔴

**存在云端（Supabase）的**：上传的原始照片/PDF（私有 bucket）、已确认的会议记录、已开出的收据、orgs 和角色、AI 用量。

**只存在那台浏览器（localStorage）的**：

| key | 内容 |
|---|---|
| `minit:money:donations:v1` | 整本捐款登记册，**含捐款人全名** |
| `minit:money:batches:v1` | 收款人→总部的托管/缴款批次 |
| `minit.constitution.v1` | 已确认的章程条文（整个问答语料库） |
| `minit.minutes.v1` | 进行中的会议记录，**连那张照片** |
| `minit.events` | 社团日历活动 |

后果：财政在手机、秘书在笔电，**看到两本不同的登记册、不同的托管余额、不同的章程**。谁清一下浏览器就全没了。`/filings` 的 eROSES paste-pack 只在拍照那台机器看得到。

附带两个问题：
- localStorage 满了时 `saveMinutes()` 会**丢掉来源照片**（唯一的稽核证据），只回一个 `"photo-dropped"`
- PDPA 的「删除组织」清得掉资料库和 storage，**清不掉每台浏览器里的捐款人姓名**

### P2 — 之后处理

- **12 张表从来没有任何程式写入过**：`extractions`、`expenses`、`constitutions`、`events_meetings`、`deadlines`、`einvois_packs`、`remittance_batches`、`paste_packs`、`committee_roster`、`qa_log`、`reminders`、`rsvps`。这些功能在画面上看起来能用，其实重开就没了。
- **`src/lib/record-upload.ts` 整段包在 `try {} catch {}` 里** —— 上传失败静默略过，使用者看不到任何错误，**稽核轨迹可以无声消失**。
- **`/search` 只抓最近 200 张收据 + 100 份会议记录**，然后在浏览器用 `includes()` 过滤。第 201 张收据永远搜不到。开支、上传、报表、活动完全不能搜。
- **`/history` 一次只能看一个月**，没有全部时间的检视。（反而 `/api/ask` 的记录搜寻做得比较好，有日期范围和 `ilike`，但要花 2 次额度。）
- **没有 `error.tsx` / `global-error.tsx`** —— 程式崩溃时使用者看到 Next.js 原始白画面。
- **`/health` 在最需要它的时候打不开** 🟠（2026-07-29 补充）
  缺环境变数时，`/health` 自己会崩掉。错误链是：
  `RootLayout` → `AppShell` → `Sidebar` → `src/components/v2/org-chip.tsx` → `getSupabaseBrowser()` throw
  （`src/db/supabase-browser.ts` 在 `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_ANON_KEY` 缺一个时直接 `throw`。）
  但 `/health` 存在的**唯一目的**就是「设定坏掉时告诉你缺什么」。设定坏掉 → 它跟着坏掉 → 完全没用。
  修法有两半，两半都要做：
  1. `org-chip.tsx`（以及任何在版面里呼叫 `getSupabaseBrowser()` 的元件）**不该 throw，应该安静降级** —— 缺设定就当作「没登入」，回传 `null`，不要拖垮整个版面。
  2. `/health` **不该套用那个版面** —— 加进 `app-shell.tsx` 的 `BARE_ROUTES`（现在只有 `/login`），或者用独立的 route group 让它完全不经过 `AppShell`。
  这跟上面「没有 `error.tsx`」是**同一类问题**：出错时没有任何安全网，一个小元件的 throw 会炸掉整页。建议一起修。
- **Claude 的沙盒连不到 Supabase —— 所有 migration 由使用者本人执行** ⚙️（2026-07-29 在旧电脑实测确认）
  沙盒对外网路是白名单制，打 Supabase 回传 `000`（连线根本没建立）。这不是设定问题，改不了。
  所以**流程固定是**：Claude 写好 SQL → 交给使用者 → 使用者贴进 Supabase Dashboard → SQL Editor → Run → 把结果回报给 Claude。
  Claude 的沙盒**不能**自己套用 migration，也无法从沙盒验证资料库现况。呼应 D8。

  ⚠️ **同日稍后更正**：以上只对「Claude 的 Linux 沙盒」成立。另外有一条路是
  **Desktop Commander**（跑在你自己的 Windows 电脑上的 PowerShell），它**连得到**
  Supabase。所以「Claude 完全碰不到资料库」这句话是**错的**，正确的说法是
  「Claude 的沙盒碰不到，但透过你电脑上的 shell 可以」。
  要不要开这条路、开到什么程度（唯读 / 可写），**尚未决定** —— 见 docs/archive/2026-07-29-进度报告.md。
- **目前只有一台电脑在动工** ⚙️（2026-07-29 补充）
  所以第 1 节「两台电脑」的注意事项**暂时不适用**，不会有 git 冲突的风险。
  实际习惯只有两个动作：**开工前双击 `pull-from-github.bat`，收工前双击 `push-to-github.bat`。**
  以后真的加第二台电脑，再回去看第 1 节。
- **AGM 文件包和银行决议书写死了 SAMPLE 水印**（`SAMPLE_UNTIL_ROSTER_INGESTION` / `SAMPLE_UNTIL_MINUTES_PICKER = true`），还不能实际使用。
- **`committee` 角色在 `src/lib/status-labels.ts` 没有标签**，画面上显示一个破折号。
- **`ai_usage` 没有 token / 成本栏位** —— 完全看不到自己的毛利。见 D2。
- **PDF 全部即时重产、不储存**。`receipts` 和 `einvois` 两个 bucket 建好了但没有任何程式写入。

---

## 5. 还没决定的事（下次要问）

1. **免费试用给 1 个真实 org，还是 0 个（只有示范组织）？**
   我的建议：**0 个真实 org + 1 个示范组织**。他们能完整试用假资料，要碰真实资料就要验证 PPM。这也符合「C 做 0」的决定，而且顺便挡住 PDPA 那个门槛（Gemini 免费层不能放真资料）。
2. **方案价格与各方案的成员席位数**
3. **分会方案的分会数上限**（试办对象是 20+ 间庙的网络）
4. **单份文件最多几页**（D3 的硬上限要一个数字）
5. **一次性试用额度给多少**（要能完成一次会议记录 + 一次收据）

---

## 6. 下一步该做什么

按这个顺序，**不要跳**：

1. **P0-1 + P0-2** —— 套用那两个 migration。先验证 `refundUsage()` 不会被 trigger 挡掉（写 vitest），再把合并好的 SQL 交给使用者贴进 Supabase。然后 `credit-form.tsx` 改成纯显示。
2. **P0-3 + D5** —— 建立 org 的上限、关掉免费层的分会权限。（需要先回答第 5 节的问题 1。）
3. **P0-4** —— 在 `src/proxy.ts` 加速率限制。
4. **`ai_usage` 加 token / 成本栏位** —— 这是 D2 和 D3 的前提，不先做就永远不知道成本。改动很小。
5. **P1-1** —— 成员邀请 / 申请加入（含 D1 的 PPM 比对）。
6. **P1-2** —— 把 localStorage 的资料搬上 Supabase。工程量最大，但这是「秘书和财政看到同一份资料」的前提。
7. **D7** —— 错误与失败追踪，然后才是完整的 superadmin 后台。
8. **换到付费 AI API** —— 收真实客户之前的硬门槛。

**每一步动手前先提计划、等确认**（`CLAUDE.md` 的工作方式要求）。

---

## 7. 这一轮改了什么

| 档案 | 改动 |
|---|---|
| `.gitignore` | 加 `.fuse_hidden*`（挡掉挂载资料夹产生的垃圾档，之前有 6 个混进版本控制） |
| `push-to-github.bat` | 改用 `%~dp0`（原本硬写 `C:\dev\minit`，**换电脑路径不同就会失效**）、commit 讯息加日期时间、自动清垃圾档、失败时给白话指示 |
| `pull-from-github.bat` | **新增**。开工前用。会先检查有没有未存的工作，有就停下来叫你先 push，**不会覆盖** |
| `docs/DECISIONS.md` | **新增**。就是这一份 |

**没有改任何程式码逻辑、没有改资料库。** 这一轮只做保存和交接。


### D9 — 定价等真实成本，先立结构（2026-08-24 J 拍板；2026-08-25 通宵落地）

**决定**：价格**等系统做好、量出真实成本再定**（J 8/22「我们看成本定」、8/24 重申）。
今晚立好的是**结构**：`src/lib/plans.ts` 是方案唯一来源（trial／standard／hq 的额度、
org 上限、功能旗标），所有猜的数字都包在 `TBD_PRICING()` 里，grep 一次就能找齐；
`orgs.plan`（migration 20260830000000）是资料库那一半。**任何画面都不准出现价格数字**，
直到真实成本量出来。计费单位从「次」换成「成本」（D1-L2）**仍然要等两周真实用量数据**，
不要提前做。额度的**显示**已全站改成百分比（F-1，2026-08-25）——那是显示层，跟计费单位无关。

### D10 — 语音：A 先做（已做），B 比赛后（2026-08-22 J 拍板）

**决定**：两个都要，**A 先**。A＝栏位口述（浏览器 Web Speech API，免费、不走 AI 额度）
——**2026-08-25 已上线**：补充框与所有自由文字核对栏位有麦克风键，语言跟 UI 偏好，
不支援的浏览器**什么都不显示**（没有死按钮），标「试验中」。
B＝整场录音转会议记录：**新的产品线＋新的敏感资料（录音）＋准确率零实证**，
放在**比赛之后**。谁要提前做 B，先重读这一条。

### D11 — e-Invois 降为选配（2026-08-24 J 拍板；2026-08-25 落地）

**决定**：大多数注册社团根本不需要 e-Invois；**eROSES 才是法定必须、放最前面**。
e-Invois 移出主导航（进「更多」），**预设隐藏**；org 级开关是 `orgs.needs_einvois`
（migration 20260829000000），今晚 UI 先用设备偏好顶住（`src/lib/einvois-pref.ts`）。
**单笔 >RM10,000 的个别 e-invois 警示不受开关影响，永远显示**——那是法定门槛，不是偏好。

### D12 — 订阅层：结构先行，金流有三个前置（2026-08-25）

**决定**：`/settings/plan` 只显示现方案、用量百分比、三档比较表，价格栏写
「定价公布前，升级请联络我们」。**禁止任何假结帐、假金流、假订阅按钮**。
真金流（Stripe 等）的三个前置，一个都不能少：
1. **真实价格**（等 D9 的成本数据）；
2. **法律实体**（个人名义收钱营运装着他人个资的系统＝无限责任，见第 5 节第 4 题）；
3. **律师看过的条款**（`legal/` 两份文件还有 `[[ ]]` 未填）。
在那之前，「升级」＝联络人类。J 手动改方案的 SQL 在 migration 20260830000000 档尾。

### D13 — 产品名维持 Minit（2026-08-24）

**决定**：**不改名**。理由：minit 是 BM「会议记录」的本字——ROS 要的东西就叫这个名；
三语使用者都好唸；比赛材料已经全部用它，赛前改名零收益、纯风险。
Fable 5 建议、J 未反对即生效。谁想改名，等比赛之后，而且要带一个好十倍的名字来。

### D14 — 防冒充 v1 的范围与两条明确「賽後、未做」的 roadmap（2026-08-25 J 批准建議②）

**做了的（本单 Stage C）**：开组织的型态栏＋选填 PPM/ROS 注册号（填了印在正式文件页首：
收据、会议记录落款区、AGM 包、银行决议摘录——DB 值为准，body 传来的一律被覆盖）；
设置页「检举冒用」联络入口（挂在 `NEXT_PUBLIC_CONTACT_EMAIL` 后，没配邮箱就不显示）；
服务条款第 8 条补冒用条款（冒名开组织＝违约、封号、留证据链、配合执法）。

**明写不做、赛后才考虑（防止有人现在做半套）**：
1. **收据 QR 查证页**（扫码显示「此编号确实由本系统开出」）——措辞要极小心，
   **绝不能读起来像「Minit 认证了这个社团是真的」**，做坏比不做更危险。❌ 未做。
2. **ROS 公开名录核对**（eROSES 有公开查询；有没有可用介面要先研究）。❌ 未做。

顺带（同一天）：条款第 9 条「被拒绝的请求不扣额度」是 8/21 之前的旧规矩，早已被
J 推翻（「用到 api 就扣」，CLAUDE.md 规则 10），法律文本却一直没跟上——已改为
「到达 AI 服务商即计费，只有完全没到达才退还」。

### D15 — 角色权限 v1：能力表放 `src/lib/roles.ts`，检查放 server action（2026-08-25 J 批准建議①）

hq_admin 全部；secretary 会议记录＋文件区；treasurer 钱全区；collector 记捐款＋自己的
交款；committee 唯读＋上传；auditor_readonly 全区唯读（连 AI 额度都烧不了）。
行事历是 v1 补充裁量：除审计外全员可写（协调活动本来就是委员的事）。
未知角色 fail-closed。逐角色 RLS 深化＝赛后。邀请码一码一人、可设过期、可撤销，
兑换走 service key（新人本来就不在 RLS 里，与 createOrg 同款设计）。

### D16 — 正式会议记录模板：模型只多标一个 kind，格式全由程式拼（2026-08-25 Stage D）

正式模板（页首 PPM＋Bil./年、出席表、Perbincangan/Keputusan/Tindakan 成文段落、
Penutup、签名栏）由 compose 层程式拼装；模型唯一多做的事是给每个 item 标
`kind`（perbincangan/keputusan/tindakan）。三个定案，动它们之前先读这段：

1. **坏 kind 只被丢弃（`.catch(undefined)`），永不让整份文件失败。** 覆盖率检查
   仍是唯一能拒绝 plan 的东西——一个标签标错顶多放错小节，不能让秘书拿不到文件。
2. **Bil.（本年第几次会议）留白 `____`，程式不数、模型不猜。** 谁都没告诉 Minit
   这是第几次会议；从历史「数出来」的期数在多设备/补录场景必错，错的期数印在
   呈报文件上比留白严重（Hard Rule 1：缺就是缺）。文件保存前可手改。
3. **中文/EN 成品页首印「翻译本 —— 非呈报用 / Terjemahan」；BM 版没有这行**——
   它就是呈报语言。UI 里公文词汇并印 BM（meetingTypeUiLabelTri）只进介面，
   **合并标签永不进文件**（有测试钉着）。

---

### D17 — 8/26 拍板整包：四卡首頁、錢區全簿記、CONTOH 禁令、配套進場、「賽後」改「上線後第一批」（2026-08-26，J 走完系統的 5 條反饋）

出處：`C:\dev\_J-要做的事\27-最後衝刺施工單-20260826.md` 第 0 節（拍板 #21–#29）。要點：

1. **首頁＝四張任務卡**（會議記錄／記錢·捐款／財報／交給 AI）＋聊天框常駐卡下方。
   「交報告」＝把會議記錄做成正式好用的報告——交不交 eROSES 是社團自己的事。
2. **錢區是整本收支簿**：收入（含實物捐贈、非捐款收入的拍照路）＋支出＋報銷
   完整批准流（交→批→付，forward-only，`src/lib/claims.ts`）＋財報
   （Penyata Penerimaan dan Pembayaran，現金制，程式加總）。
3. **實物捐贈**：同一連號序列；收據印品項不印錢；估值選填、只入帳；
   不進 e-Invois、不進現金交款；amountCents 一律 0（防呆）。
4. **側欄七組攤開**（桌面），組名不可點、恆展開；**手機底欄 4 格不動**；
   /more 用同一分組來源。
5. **CONTOH 禁令**：示範是「另外的東西」，永不擋住或污染真用戶的產出——
   AGM 包/銀行摘錄 roster 空＝誠實空狀態指去成員頁；示範走明確標示的獨立入口、
   用虛構社團自己的名字。
6. **配套進場**：開組織選配套（試用預設；標準/總部＝記下 orgs.plan＋人工開通，
   無假價格無假金流，開通前額度照試用 15）。
7. **「賽後」禁語**：能做的都做。真不做的只有兩類——要 J 在場/提供的
   （上線、真手寫、eROSES 逐欄），和要正式網址才有意義的（QR、SMTP、RLS 深化）
   ——後者叫「上線後第一批」。

---

### D18 — 捐款人姓名：畫面上預設顯示全名；「隱藏姓名」只在列印/分享/截圖時作為選項（2026-08-27，31 號單 §0 拍板 35）

姓名是用戶自己打進去的，記錄系統要看得到是誰——列表預設打碼讓司庫自己都認不出
自己剛記的人。**修訂 Hard Rule 5 的「donor personal data is masked in list views
by default」那一句**（CLAUDE.md 已就地注記指到這裡）。PDPA 底線不動：AI 模型仍然
永不收到完整姓名/電話（org-tools 不 select `donor_name`/`donor_phone`）、對外分發
（列印、WhatsApp、截圖）才是遮蔽選項該出現的地方、donor 資料永不入 log。

### D19 — 每筆收入登記時就問「現金／轉賬」；「交現金」改為保管記錄頁（2026-08-27，31 號單 §0 拍板 34）

`donations.payment_method`（'cash'|'transfer'，default 'cash'）＋
`donations.transfer_proof_path`（選填轉賬截圖，Storage 不吃 AI 額度）＝ migration 26
（Stage B，唯一授權的一支）。現金記「在誰手上／已交總會」；轉賬單不經現金保管
（不進交接、不算「手上」）。「交現金」頁從流程第 3 步改成**保管狀態記錄頁**
（誰手上有多少、他交來了→打勾確認、歷史）；步驟條 1-2-3 改 1-2。
custody 狀態機（forward-only）語義不變。

### D20 — 品牌改名槓桿：顯示名集中一個常數；8/31 提交前不改名；已開收據字首鎖死（2026-08-27，31 號單 §0 拍板 38）

`src/lib/brand.ts` 的 `BRAND_NAME` 是唯一的用戶可見顯示名來源（Stage E 接線）。
MinitAI／Minity 賽後再定（D13 維持到賽後）。已開出的收據字首（MIN-2026-xxxx）
**永不改**——連號的法律意義高於品牌一致性；改名真的發生時，只有新機構用新預設字首。

### D21 — e-Invois 下載檔要「直接可交」：說明文字不入上傳檔（2026-08-27，31 號單 §0 拍板 37）

MyInvois Batch Upload 的 .xlsx 只放資料表（Dokumen 表第一且唯一，或完全照官方模板
形狀）；「Arahan - Instructions」工作表移出上傳檔，說明改成頁面上的步驟卡＋（可選）
獨立 README 下載。等 J 提供官方模板原檔後逐欄對齊；對齊前檔上保留
「verify against LHDN template」警語。（Stage B-7 執行。）

### D22 — 表單規範：必填＝紅色星號；選填＝什麼都不標（2026-08-27，32 號單 §0 拍板 0-3）

必填欄位在標籤後加紅色 `*`（共用組件 `src/components/required-mark.tsx` 的
`<Req />`）；選填欄位**不加任何尾巴**——「（可不附）」「（可選）」「(pilihan)」
「(optional)」全部刪除。理由（J 原話的意思）：「可不附」這種尾巴讀起來像在
引導（「你最好還是附」），而且三語各寫一次、寫法不一。漏填必填欄時，就地紅字
說明哪一格、為什麼。標了紅星的欄位不得再有靜默 fallback（例：收款人空白時
不再默默用預設名頂上，改為就地報錯）。

### D23 — 名字定案：MinitAI（2026-08-27，32 號單 §0 拍板 0-1）

`BRAND_NAME = "MinitAI"`（brand.ts 一行）。D13「賽後才定名」被此條覆蓋；
D20 的其餘部分不變：已開收據字首 MIN- 鎖死、prompts 一字不動。

### D24 — 紫色全套改版定案（2026-08-27 下午，J：「logo 用這個，design 換差不多這樣，但功能還是用我們自己的」）

**覆蓋當天早上的拍板 0-8（綠色）**。以 tester（tan shi hui）的紫色規格書為底：
主色 `#7029E5` 系、紫色雙人 M logo、Poppins、icon rail 側欄、sticky 玻璃頂欄
＋頭像選單（機構切換收進去）、分屏登入、設置 13 子頁＋子側欄、五級圓角、
淡紫漸層 canvas。**功能、導航分組（拍板④）、金流、RLS 一律用我們自己的**——
換皮不換骨。全套一次做（J 拍板「全套現在做」，8/31 交件前）。
偏差一條：輸入框邊框保 ≥3:1（規格書的 1.55:1 會重演「看不到框」）。

### D25 — eROSES 文件的馬來語把關（2026-08-27 下午，J）

要交 eROSES 的馬來文文件**不可以有華語**。程式確定性偵測（免費）、就地標紅
＋鎖保存；出路由**用戶選**：「讓 AI 譯成正式馬來文（花額度，走現成
draft-minutes 路）」或「自己改」——不背著人自動翻譯。機構**註冊名照印、
永不改寫**（不擋自己的文件）；人名用名冊 IC 官方名。四種文件都要正式：
會議記錄（Stage D 模板＋此把關；深版式仍等 D-8）、財報（PDF 簽名欄）、
貼上包（逐格標紅）、AGM 包（地點就地把關）。

---

### D26 — 錢先動，收據跟著（2026-08-27 晚，J 20 條 #4：「都是先交錢才開收據……先拿到錢才開收據才對」）

交現金**不再要求先開收據**。交接批次本身（捐款人＋金額＋日期＋經手人）就是憑據；
已有收據號的行照樣帶號。批次改用 `client_donation_ids`（migration 28）直接連行，
不再只靠收據號解析；pre-28 資料庫遇到含未開收據行的批次會誠實拒寫（「只在這台
設備」），絕不無聲丟行。custody 狀態機 forward-only 照舊（D19 不動）。

### D27 — 一輪一個流程（2026-08-27 晚，J 20 條 #3：「那一輪就是那一輪的東西，不會跳來跳去」）

錢的流程以「本輪」為單位：/money（收錢記賬：拍賬頁＋打字＋手動，全部入口在這）
→ 本輪清單就地雙重核對 → /money/issue 只為**這一輪**開收據 → 完成開新一輪。
/money/receipts 退為**管理頁**（篩選有無收據、多選開、重新下載）。捨棄的舊形：
「下一步」跳到混著全部記錄的開收據頁。

### D28 — 導航與詞庫的三條（2026-08-27 晚，J 20 條 #2/#7/#10/#12/#15）

① 側欄組是 **dropdown、預設收起**，所在組自動展開，手動開合記在設備上；
② 日曆是頂級行（活動＋提醒，不屬申報）——覆蓋 §1-9 的「收回申報組」；
③ /settings 內**主側欄隱藏**，設置欄自帶「返回」＝唯一側欄；系統檢查只給
manage_org；④ 詞庫改「原文＋它是什麼語言＋另外兩種語言的叫法」（任何語言都可
以是原文；兩個叫法全空＝保持原字）——舊 action/translation 照寫（BM 叫法＝
translate），prompt 路一字不動；⑤ 副歷（農曆／伊斯蘭曆）**opt-in**，格子預設
純西曆；農曆初一/十五重複活動由程式展開（lunar-parse），標題用社團自己的字。

---

### D29 — Prompt 解凍：主功能優先於量測（2026-08-28，用戶＋J 雙拍板）

**推翻「助手/提取 prompt 一字不動、動了量測作廢」的凍結。** J 27 晚 #30 原話：
「不要管之前的測試，我會重新測試 API usage，主要功能先做好！之前説了好幾次了！」
用戶 8/28 拍板：「解凍，我從之前就一直說了。有些規矩需要被推翻才能進步！」
生效範圍：所有 prompt（會議記錄提取、決議結構化、職位偵測、問一問）都可以為了
準確度改。代價明寫：既有 bench/eval 數字全部作廢，改完要重測（J 說了他來重測）。
「動 prompt 之前先問這讓哪些量測作廢了」這條紀律**仍然有效**——答案要寫出來，
不是不准動。

### D30 — 出席人數不得為零就出報告（2026-08-28，J 27 晚 #33）

「These notes do not record attendance」逃生口**拆除**。出席是 eROSES 年度呈報的
法定欄位（Bilangan Ahli Hadir）：沒有出席記錄就不能進入成品頁，唯一的緩衝是
「稍後補上（will insert later）」——選了它，文件一路標「出席待補」水印，
補齊之前不算「已確認」。反轉 32 號單時代做的逃生口設計。

### D31 — 現有資金頁（2026-08-28，J 27 晚 #24）

新功能：一頁看「現在還有多少錢」（收入−支出的現金制餘額，程式加總，Hard Rule 2）。
金額預設用眼睛圖標遮住、點了才顯示；只做在這一頁，不進財報、不進首頁卡。
權限：treasurer / hq_admin（錢區既有 can() 口徑）。

### D32 — 記錄即入庫：登記的每一筆錢當場進資料庫（2026-08-28，錢區 #17 重複交接 bug 的根治）

J 27 晚 #17 的根因：交接/確認只改本機 localStorage，DB 的 `donations.custody_status`
永遠停在開收據那一刻，而合併規則「同 id DB 覆蓋本機」每次載入把已交接的行復活成
「可交」→ 同一張收據交兩次、確認記錄成雙。定案三件：
① **記錄一筆收入＝當場寫 DB**（migration 29 的 `save_register_rows`，upsert on
org_id+client_id；`issue_receipts` v8 同支 migration 學會「行已存在→補開收據」，
兩件事同一支 migration 出貨，不留中間窗口）；localStorage 降級為離線草稿，寫不進
DB 時照舊明說「只在這台設備」。
② **交接/確認/取消批次時同步回寫成員行的 custody_status**（純 PostgREST update，
不需 migration，任何庫齡可用）。
③ **合併規則改為 forward-only 勝**：同 id 兩份資料，custody 狀態取走得最遠的
（collected < pending_remittance < settled）；批次同理（pending < settled/cancelled）。
另：settled/pending 批次的成員行在載入時由批次推導鎖狀態（self-heal，J 現有的
壞資料下次載入自動歸正；殘留的重複 pending 批次由 J 手動取消，settled 批次照
規矩永久鎖存不清理）。

### D33 — PdpaNote 隱私提示整個刪除（2026-08-28，J 對 37 號報告 §6-1 的回答：「刪除」）

「🔒 照片只交給 AI 服務商讀取」那段（含展開詳情）從四個掛載點（首頁 AskBox、
會議記錄、記賬、章程）連同元件一起刪除。法律層面的隱私承諾仍在 /privacy 法律文；
被刪的只是介面上的重複提示。若日後法規或商店審核要求介面內提示，重新掛回是
一個元件的事——但要 J 再拍板。

### D34 — 逐區塊 AI 討論（2026-08-28，J 對 37 號報告 §6-2 的回答：全部區塊要、改一次算一次）

核對頁四個區塊（會議資料/決議/金額說明/職位名稱）各有「跟 AI 討論」。計費：
**每發一次訊息收 1 個 AI action（`discuss_minutes`）**，成本寫在按鈕上。
鐵律不動：AI 只「建議」逐行改寫，人按「採用」才生效（走同一條 updateField 路，
source_ref 記「AI 建議，您採用」）；人名與金額 prompt 與程式雙層禁改——人名
歸名冊的 IC 姓名頂替按鈕（免額度、程式替換），金額歸記賬。

### D35 — 會議記錄有名字、可列印、可修改、連回原始照片（2026-08-28，J 新七條 #3/#4）

migration 30（`20260908000000`）：`minutes_docs` 加 `title`（保存前問名字，
Google-Docs 式建議名＝類型＋日期，程式生成零額度）、`edited_at/edited_by`
（歷史頁可就地修改，**每次修改文件末尾自動加一行「Dipinda oleh 誰 pada 幾時」**
——J 的原話「做修改下面就要寫幾時 EDIT」）、`photo_paths`（extract 時本來就存進
uploads bucket 的原始照片，終於連回它的會議記錄；歷史頁縮圖＋signed URL）。
列印＝`/api/minutes-pdf`（A4、CJK 字型、pdf-lib）——同一份 PDF 就是 eROSES
「Muat Naik Minit Mesyuarat」上傳框要的檔案。DB 落後時全部欄位自動降級（梯度）。

### D36 — 保存即成品頁；已存檔的工作區即棄（2026-08-28 晚，J 八條 #1/#2/#6/#7）

「保存到歷史」成功後**直接落在成品頁 `/minutes/history/<id>`**：最終預覽、
🖨 打印/PDF（即 eROSES 上傳檔）、原始照片（縮放彈窗）、✏️ 就地修改（留痕照舊）
全在同一頁。歷史列表每份記錄的名字就是入口（點名字＝打開成品頁）。
配套：**已存進歷史的工作區不再還原也不再逗留**——重新進 /minutes（或本次會話內
點「新的會議記錄」）一律是乾淨的新工作區；「上一場已存好」卡片與「重新打開工作區」
路徑刪除。要改已存的記錄，走成品頁的「修改」。未保存的工作區照舊完整保留。
背景：J 兩度報「開新的會議記錄還保留之前的」為 BUG；舊設計把「保存後的殘留」
當功能（補拍同場另一頁），J 的用法證明它只造成困惑。

### D37 — AI 撰寫允許「同類項合併」，代價由程式當場收（2026-08-28 晚，J 八條 #4）

draft-minutes 的 plan schema：`source` 可為索引**陣列**——十三行「某班 N 位」
可合併成一句可讀的正式句。三道算術照舊/新增：① 覆蓋檢查不變（每個索引全計劃
恰好出現一次）；② checkNames 照舊（合併行的中文 run 須出自其任一來源）；
③ **新 checkMergedFacts（所有語言都跑）**：合併行必須仍含每個來源的
「兩字以上中文詞＋所有數字」（行首「1.」序號與單字量詞 位/個 可去）——
掉一組名字或數字→退回重寫一次，仍失敗→退款不出文件。
另：快速預覽對「自帶編號」的筆記行照印原文，不再套第二層編號（「2. 1. 宏道」修掉）。
⚠ 真實 vendor 行為未驗證——規則在 prompt＋schema＋三道檢查裡，J 下次用真額度
按「讓 MinitAI 寫成正式記錄」即是首次實測。

### D38 — 圓角只剩五個值，而且整條尺度往上挪一階（2026-08-28，美術三包 01）

`--v2-r-xs/sm/md/lg/pill = 6/8/12/16/999`，寫在 `:root`，`@theme` 的
`--radius-*` 全部改成 `var()` 查表——**全 app 沒有一個元件自己寫圓角數字**。

🔴 **Tailwind 的階名意義整體上移一階**（`rounded-md` 從 8px 變 12px）。
理由是這個 repo 本來就是這樣用的：按鈕掛 `rounded-sm`、卡片和面板掛
`rounded-md`、彈窗和登入卡掛 `rounded-lg`。挪尺度＝每一族剛好落在該落的值，
**不必動 340 個呼叫點**。卡片 12px 是錨，其他都對著它判斷。

輸入框例外，用一條不分層的元素選擇器釘在 8px：這個 repo 的輸入框和卡片共用
`rounded-md`，要分開就得逐一分類 340 處，而且下一個新輸入框還是會寫錯。
巢狀規則 `inner = outer − padding`（12px 卡片配 1rem padding → 8px 輸入框）。
另外兩個宣告過的例外：checkbox 4px、3px 導覽指示條。
⚠️ radio **不**在 4px 那條裡——圓形是普世慣例，方掉會看成 checkbox。

順手修掉的既有錯：`.v2-pill` 是不分層 CSS，它的 border-radius 一直壓過
`rounded-full`，所以 J 8/27 指定「圓形」的浮動 AI 按鈕其實一直是圓角方塊；
tab 的圓角比裝著它的 tab list 還大（12 vs 8）。

### D39 — 畫布漸層一直有畫，只是被蓋住：透明的是 body 和 .v2-root，底色在 html（2026-08-28，美術三包 02）

`--v2-page-grad` 早就定義好、`.v2-root::before` 也早就在畫，但它是
`z-index:-1`。CSS 2.1 附錄 E 的繪製順序：**負 z-index 子層（第 2 步）比
in-flow 區塊背景（第 3 步）先畫**。所以 `body { bg-background }` 和
`.v2-root { background: var(--v2-page) }` 兩層都蓋在漸層上面。

定案：`--v2-page` / `--v2-page-grad` 搬到 `:root`／`.dark`（`<html>` 讀得到），
`html` 拿平色當底、`body` 與 `.v2-root` 都 `transparent`。
🔴 **不要把任何一個改回不透明**，改一個漸層就再消失一次。
平色留著是真的後路：`prefers-reduced-transparency`、強制色彩、殼外的路由。

### D40 — 首頁四張卡：四個顏色只准碰三樣東西；狀態行分「讀不到／是零／有數字」三種（2026-08-28，美術三包 02+03）

卡片從 2px 近黑外框改成 1px `--v2-border` ＋ 陰影 ＋ 12px 圓角 ＋ 頂上 4px 色帶
＋ 圖示磚 ＋ hover 抬 3px 滑出箭頭。Emoji 全刪——📝📋📊✨ 由作業系統畫，
產品的第一個畫面在 Windows／Android／iOS 看起來像三個不同產品；改用專案本來
就有的 lucide 線條圖示。

**`--c` 只准出現在三個地方**：4px 色帶、圖示磚（底色＋字符）、7px 圓點。
邊框、卡底、標題、說明、狀態文字一律 `--v2-border` / `--v2-card` / `--v2-text`
/ `--v2-text-soft`，否則四個色就變成四套主題。圓點是裝飾，語意由旁邊的字扛，
色盲讀者不會少任何資訊。淺色四個色都是深色，深色模式另給四個亮版（不是
color-mix 推導，舊 Android WebView 也要一樣）。

🔴 **狀態行的三態規則**（`src/lib/home-card-lines.ts`，13 支測試釘住）：
`null` 讀不到 → **整行不畫**（不是 0、不是橫槓、不是轉圈）；
`0` 讀得到而且真的沒有 → 寫成邀請（「還沒有會議記錄 —— 拍一張筆記就開始」）；
`n` → 數字。剛成立的社團和壞掉的查詢，只看 falsy 是一模一樣的；把「你沒有記錄」
講給查詢失敗的人聽，是合規工具最不該犯的錯。同理 `moneyRecords` 是
`{ latestMonth } | null` 的雙層 null——外層 null＝讀不到，內層 null＝真的沒有。

登入頁同場：白色線稿 M 換成真正的 app icon（`BrandLogo` 的磚塊版，本來就有向量），
直接放在漸層上、不加白底不加外框。分離靠 `filter: drop-shadow()`
**不是 box-shadow**——PNG／SVG 磚塊外面是透明的，box-shadow 會畫出一個方形陰影，
圓角就爛了。陰影是深紫 `rgba(35,12,74,.16)` 不是黑（黑色會把下面的紫去飽和，
看起來像髒污），單層 16%：要更分離就加模糊半徑，永遠不要加黑。
品牌字級大於「歡迎回來」——登入頁的人已經知道自己被歡迎，要確認的是站對了產品。

### D41 — 圖示要過「馬來西亞每一個族群」這一關；豬撲滿刪除（2026-08-28，J）

J：「**不要用豬圖，因爲有馬來人用。要注意每一個種族的sensitive**」。

`/money/balance`（現有資金）本來掛 lucide 的 `PiggyBank`。豬對這個產品的主要
使用者（穆斯林）是 haram 意象，側欄圖示不是讓人皺眉的地方。已換成
`CircleDollarSign`。（中間試過 `Vault`，在側欄尺寸下轉盤看起來像一個 ✕，
會被讀成「取消」，退回。）

🔴 **這是一條常設規則，不是一次性修正**：以後每加一個圖示、emoji、插圖或範例
照片，都要先對照馬來西亞的族群與宗教讀一遍再上線。已掃過的現況：
- src 全域沒有豬／酒／狗／宗教符號的 emoji（唯一的 🙏 在 WhatsApp 出席確認
  訊息裡，當「請／謝謝」用，馬來語圈普遍使用，留著）。
- 範例姓名三大族群都有（Lim/Tan/Wong、Siti/Ahmad/Nurul、Muthu/Kumar/A-L），
  沒有偏向任何一方——新增範例資料要維持這個比例。
- `Landmark`（AGM 文件包）是通用機構建築，不是任何宗教建築。

### D42 — 品牌標記只有一份，而那一份是 **J 給的原圖**（2026-08-28，兩次才對）

J 指出瀏覽器分頁的圖示跟 App 裡的 logo 不一樣。查出來的原因不是「忘了更新」——
**本來就是兩張畫**：favicon／PWA／iOS 圖示是從 J 給的
`scripts/assets/minit-logo.png` 產的（**正本**），而 App 裡畫的是前人做的
**向量仿製版**（`brand-logo.tsx` 的 inline SVG），線條較細、紫色較飽和。

🔴 **第一次修的方向是錯的**：我把兩邊統一到**仿製版**，等於把 J 的正本從
分頁和 PWA 圖示上換掉。J 當場抓到：「MinitAI 的 LOGO 應該是這個，爲什麼你換了呢」。

**定案：正本是唯一真相，仿製版刪除。**
- `scripts/assets/minit-logo.png` 是來源，不准用別的東西產生這個標記，也不准重畫。
- `scripts/brand-icons.ts`（`npm run icons`）從它產出 favicon.ico（48/32/16）、
  icon-512／192、apple-touch-icon（180，壓平在 #7029E5，iOS 不吃透明）、
  brand-logo-96。管線：trim 掉白邊 → 補正方 → 22% 圓角遮罩把四角切成透明
  （白角在深色分頁上會很醜）。
- `BrandLogo` 只是 `<img src="/icon-192.png">`。**不再是我們維護的一張畫。**
  清晰度靠「用 192px 的檔案塞進 28–64px 的框」解決（最大用途是登入頁 64px，
  3 倍螢幕也夠），不是靠重畫。用 PWA 那張是刻意的——同一張圖同一個尺寸，
  再存一份只是多 67KB 一模一樣的位元組。
- `src/lib/brand-mark.ts`（仿製版的幾何）已刪除。

換 logo 的流程：換掉那張 PNG → `npm run icons`。就這樣。
順帶：登入頁和重設密碼頁都改用磚塊版（J：「統一」），`BrandLogo` 的 `white`
線稿變體也一併刪除。

⚠️ 分頁圖示的快取特別頑固：改完要 Ctrl+Shift+R 兩次或關掉分頁重開才看得到。

### D43 — 全站不准有黑色按鈕，`--primary` 就是品牌紫（2026-08-28，J）

J：「裏面不要有任何黑色的按鈕，都換成紫色的」。根因是 shadcn 預設的
`--primary` 一直是炭黑而本 repo 從未改過它。定案：`--primary: #7029E5`
（白字 6.69:1）、深色 `#7C3AED`（5.70:1）；`text-primary` 類文字用途改走
`var(--v2-primary)`（填充紫當文字在深色卡上看不見）。src 裡的 `bg-black`
全是遮罩／燈箱底，不是按鈕，一個都沒動。驗證工具
`scripts/shots-no-black-buttons.mjs` 逐頁檢查實際背景色。
（本條做在 2026-08-28 下午場，當時漏了補進這份檔，2026-08-28 晚補記。）

### D44 — 免費圍欄：終身 5 份文件・20 張收據・20 頁・3 次乾淨下載（2026-08-28，J）

J 的原話拆出來的四條圍欄，全部**終身制**（用完不重置；刪掉東西**不退次數**
——數的是「做過」，不是「現存」）：

| 圍欄 | 免費版 | 計數點 |
|---|---|---|
| 文件 | **5 份** | 會議記錄保存進歷史＋文件包（AGM/財報/銀行摘錄/e-Invois）的**乾淨**產出 |
| 收據 | **20 張** | 開立時擋（receipts 表 gap-free 不可刪，count(*) 就是真話） |
| 上傳 | **20 頁** | AI 讀的頁數：照片 1 張=1 頁、PDF 一頁=1 頁；貼上的文字不算 |
| 乾淨下載 | **3 次** | 只管文件線；**收據永遠乾淨、永遠不吃這 3 次**（不然送 20 張是假的） |

**「看得到、拿不走」**：免費版所有文件預覽（畫面上的全文、PDF 檢視、打印）
一律壓 PERCUBAAN 浮水印＋禁複製；乾淨檔只從計次的「乾淨下載」出去。
截圖擋不住，但截走的也帶浮水印——鎖因此是真的。

- 終身制是 J 自己的理由：「一個月其實也只開 1-2 次會議，只能靠錢記錄那邊賺取」
  ——按月重置等於永遠用不完；轉付費的引擎是錢區（收據 20 張）。
- **誰被圍**：不是看 plan 字串（開組織時自選 standard 不算付費）——看
  `monthly_free_quota > 15`（開通=J 手動 SQL 抬 quota，跟 /settings/plan
  的「已選未開通」判斷同一條算術）。demo org 永不圍（CONTOH 禁令）。
- 讀取失敗**開著失敗**（資料庫打嗝不准給付費社團蓋浮水印）；計數失敗
  **關著失敗**（數不了就不交乾淨檔——跟 AI 計量同一條誠實規則）；
  fence_charge 不存在（migration 沒套）＝完全照舊（D8）。
- 廠商沒到達就退頁數（與 ai_usage 退款同義，退款路一一並排）。
- 上限數字只寫在 `src/lib/plans.ts`（PLANS.trial.fence）；資料庫只記發生過
  什麼（fence_usage 表＋fence_charge()，migration 20260909000000，內含把
  J 的 15/58/91 三個 org 標成 standard/quota 100）。
- 價格仍未定（D12 照舊）——付費按鈕依然是「聯絡我們」。
- 章程撞牆的後果已由 **D45（A6 特例）** 開口子：一份章程最多扣 5 頁，
  見下一條。

### D45 — A6 特例：章程上傳一律扣 min(實際頁數, 5) 頁（2026-08-28 拍板，2026-08-30 再確認，81 場落地）

D44 的終身 20 頁上限撞上一個事實：章程常常 20–40 頁，免費版基本傳不完
一本完整章程。J 在 50 號檔第 31 行拍板「弄好來」（2026-08-28），
2026-08-30 追問時再確認——**開這個口子**：

- 章程（三個門：/constitution 頁、開機構上傳、首頁 intake 分類到章程）
  一律只扣 `min(實際頁數, 5)` 頁；4 頁收 4、5 頁收 5、30 頁收 5
  （`src/lib/constitution-pages.ts` 的 `constitutionFencePages`，測試釘住）。
- **分段讀（81 場 I1）之下一份章程只收一次** min(實際,5)——不是每段各收
  一次。*（本條的 fence 部分照舊有效；「整份章程只收會員 1 個 extract
  action」那半句 2026-08-30 晚被 **D47** 取代——extract action 改按頁數
  公式扣、跟著讀進度走。）*
- 為什麼是 5：等於免費版「一份文件」量級的頁數；章程是一次性的立檔動作，
  不是持續消耗，把它擋在門外只會讓免費版永遠體驗不到「問章程」。
- 其他文件（minutes/ledger/roster）**照舊按實頁數扣**——引流引擎在錢區，
  口子只開給章程。

### D46 — 收據 QR 查證頁的三條鐵線（2026-08-30，87 場落地；24 號單建議② 的賽後項）

每張收據 PDF 印一個 QR，指向公開頁 `/verify/resit?t=<HMAC 簽名 token>`。
24 號原話「做壞比不做更危險」落成三條鐵線：

1. **只認 QR token，永不開「手動輸入編號查詢」。** 收據編號連號無缺口
   （Hard Rule 2）——一個純編號查詢框等於讓任何人枚舉全庫收據。有簽名
   token 才有查詢；沒有 token，查詢面積是零。token 無期限（紙會活七年，
   過期的 QR 會讓真收據看起來像假的——防冒充功能最壞的失敗方向）。
2. **頁面只覆述紙上本來就印著的欄位**：org 名、日期、金額（實物收據
   顯示「實物捐贈」不顯示品項——自由文字欄可能載人名）。verify 路的
   查詢永遠不 select `donor_name`/`donor_phone`/`purpose`/`item_desc`
   （Hard Rule 5）。
3. **絕不能讀起來像「MinitAI 認證這個社團是真的」**：頁上三語明寫
   「只證明編號出自本系統，不證明社團身份」，PDF 的 QR 旁也印同一句；
   查無此號＝「本系統沒有開過這個編號」＋檢舉冒用管道；資料庫打嗝＝
   「現在查不到，稍後再試」，**永不**把打嗝講成「收據是假的」。

零 migration（學 81 場續讀令牌：HMAC 從部署密鑰派生、domain-separated）。
舊收據沒 QR 照舊有效——沒有 verifyUrl 就不畫 QR，版面一字不變。

### D47 — 章程讀取扣費公式：頭 20 頁每 5 頁 1 次、第 21 頁起每頁 1 次（2026-08-30 晚，J 拍板；89 場 ⑧ 落地）

**取代 81 場（D45 記載）的「整份章程只收會員 1 個 extract action」**。
J 原話：「不然他丟 50 張我們就半死，我們也不是做慈善」——一口價 1 次
讓 50 頁的書跟 4 頁的收一樣，而 vendor 是按頁跟我們收的。

- 扣次數 = `ceil(min(N,20)/5) + max(0, N−20)`。
  釘住的數：4→1、5→1、8→2、20→4、21→5、50→34
  （`constitutionReadActions`，constitution-pages.test.ts）。
- **收費跟著讀進度走**：每個 segment 只扣它自己那幾頁「新增」的次數
  （`constitutionActionsDelta`；落在已付的五頁塊裡的 segment 扣 0）。
  讀壞的 segment 退它自己扣的；續讀令牌帶 `pagesDone`——已讀的頁
  永不重扣。令牌換形狀＝舊令牌（30 分鐘 TTL 內的）驗不過，客戶端
  自動退回「重新收費的新讀取」，誠實不白扣。
- vendor 成本照舊全部累計在第一行 ai_usage（seed 機制）＝「我們付了
  多少」一個數；後續 segment 扣的行是會員側的量表，cost 留空。
- **A6/D45 的免費圍欄頁數 min(N,5) 一字不動**——那是另一個表；
  「先講價」那行（86 場 ④）改寫成「會扣 X 次 AI 用量」。demo 路照舊零扣。

### D48 — Members 必填雙硬擋：表單也擋、申報也擋（2026-08-30 晚，J 拍板「都要」；89 場 ⑦ 落地）

**推翻 2026-08-19 的「只在申報咬人、不在加人時擋」**。eROSES 必填欄
（姓名、IC 名、州屬、任命日期——以 H2 對過的 portal 欄為準；電郵不擋）：

- **表單硬擋**：/members 的新增與編輯，缺任何一格不給存，拒絕句點名
  缺哪格（`erosesCommitteeRefusal`，三語）。
- **申報硬擋**：penyata flow 第 3 步（AJK）任何一行有缺，Senarai AJK
  的 COPY 鎖住（值 select-none，同 §1-11 真鎖），缺誰缺什麼列在原地，
  就地補（`fillCommitteeErosesGaps`，H2「補一格」的路放寬到四欄）。
- **舊資料不炸**：歷史行、名冊照片/匯入、種職位骨架照舊可以留缺——
  只是整行 amber、行內點名缺什麼、必填欄掛 eROSES 徽章、頂部 banner
  一鍵捲到第一個缺行。migration 37 未貼時 state 欄讀不到＝不擋（D8）。
- ⚠ **風險註記（工單要求白紙黑字）**：表單硬擋正是 2026-08-19 那條
  舊拍板要防的局面——秘書手上只有中文名，被表單逼著當場自己音譯
  亂填（68 場模型音譯前科：En.Loo Sio San 被發明成「吕兆生」；人手
  音譯是同一種病，而且進的是政府表格）。對策是擋不鬆、警語常駐
  （「照 IC 抄，不要自己音譯」貼著每個 IC 名輸入框），上線後要盯
  真名冊有沒有出現可疑的「當場拼出來」的 IC 名。

### D49 — e-Invois 整區進 BETA 閘（operator-only）＋ financial_resolutions 欄位（2026-08-30 深夜，J 拍板；94 場落地）

背景兩條：①93 號 §1-4 拍板「MyInvois 對齊暫停；正式版把 e-Invois 藏起來」
——LHDN 官方欄位對齊沒做完之前，一個看起來像稅務工具的半成品比沒有更危險；
②partner 交來 e-invois-governance 包（批款→e-Invois 審計狀態的純邏輯＋
成品頁面板＋extraction 新欄 `financial_resolutions`），J 8/30 拍板
「既然有了，就直接做」＝包照收、閘同場落地。

- **閘的形狀**：`EinvoisProvider` 多一個 server 算好的 `operator` bit
  （`isOperatorEmail`，與 /admin、/health 同一份 ADMIN_EMAILS 名單），
  `visible = operator && (org 開關)`——所有 `useEinvoisVisible()` 消費點
  （側欄、/more、home-upcoming、日曆截止日、成品頁新面板）一處全閘。
- **頁面與 API fail-closed**：/money/einvois、/settings/einvois（開關頁
  本身也是藏的對象）對非 operator 一律 404；`/api/einvois-xlsx` 伺服器端
  同擋——**取代**舊註解「route 永遠能走、藏選單不斷書籤」：BETA 藏起來
  就要藏得住，被存的連結打得開就不叫藏。
- **免費層跟閘走**：兩條 e-Invois 預備問答＋「Apa itu e-Invois?」chip
  在閘後不出（chip 永不扣費是 K1 鐵律，閘後那顆會漏到模型去）；
  /api/chat 的 money_einvois 深連結按鈕對非 operator 不回（答案文字照舊）。
- **operator 看到的每個 e-Invois 入口掛同一顆 BETA 徽章**
  （`EinvoisBetaBadge`，tooltip 三語講明只有平台管理員看得到）。
- **org 的 `needs_einvois` 值一字不動**：閘是蓋布，不改資料——解閘＝
  拿掉 operator bit 的 AND，各 org 自己的開關原樣回來。
- **financial_resolutions**（partner 包）：extraction schema 新可選欄
  （optional+catch，舊檔不炸）＝會議「批款」的結構化視圖（收款方/金額/
  用途，逐字照抄）；判斷全在 `einvois-governance.ts` 確定性代碼（門檻
  RM10,000 引 einvois.ts 既有常數；章程上限只認貴會自己的條文，讀不到
  ＝不出 finding，絕不發明全國上限；無任何狀態聲稱 LHDN 已驗證）。
  prompt 明文允許同一決議在 resolutions 與此欄各記一次＝「一實一記」的
  唯一例外，白紙黑字。

### D50 — 「交現金」方向：交接狀態長在收入記錄上，不再是獨立頁（2026-08-30 深夜，J 拍板；97 場記錄方向、側欄先收）

J 8/30 拍板：Hand over cash（/money/custody）從側欄藏起。97 場只收門牌
（側欄行拿掉；路由、custody 引擎、狀態機、全部測試一字不動）——方向是：

- 日後交接不再是一個要「另外去一頁做」的事，而是**收入記錄上的一個
  交接狀態**＋「一鍵已交總部」；custody 的 forward-only 狀態機
  （collected → pending_remittance → settled）原樣沿用，只換 UI 的家。
- 動工前先讀 D32（本機記錄 vs 機構記錄）與 §6 的 forward-only 合併
  教訓——那兩條就是這個引擎踩過的雷。
- 在那一天到來之前 /money/custody 照常能用（直達連結、書籤都活著），
  只是選單不再引人進去。

### D51 — Home 就是 AGENT 工作台；放鬆／鎖死線與兩級改動制（2026-08-31 早，J 親口拍板；100 號單 §0）

J 8/31：「可以收掉了，因為不需要了，左邊也有 sidebar 可以使用了」＋
「所有的限制都弄鬆，只要不傷害到我們」。

- **Home 四張卡刪除**，整頁＝agent 工作台（對話＋拖放＋步驟可見＋
  多成品卡片＋⑨自我報告）。側欄每一頁保留，不習慣 agent 的人照走舊路。
- **放鬆（形式自由）**：措辭／版式／重組；主動追問；一次對話多份成品；
  聊完直接改稿（discuss 面板從「逐條手動套用」改為「直接套＋舊→新＋
  還原鍵」，套上的行標 `check`，正式文件前仍須人過目＝確認閘沒動）；
  可建議缺漏但必須標明是建議。
- **鎖死（傷到我們的，逐字在 `src/prompts/agent-soul.ts`，測試釘住）**：
  錢永遠 TypeScript 算；收據編號不可改；donor 全名/電話永不進模型；
  不代按確認、不代交 eROSES；不給法律稅務會計意見；每場對話有成本上限。
- **兩級改動制**（「member 換手機號碼」那題）：可復原的普通改動
  （電話/email/州屬/敬稱/備註）agent 直接改＋「已改：舊→新」＋還原鍵＋
  `agent_changes` 留痕（migration 41；**留不了痕＝不改**，圍欄同款
  fail-closed 方向）；不可逆／錢／寄出／刪除／eROSES 提交＝agent 備好、
  人一個 tap 確認。
- **常駐三語小字**（Anthropic 同款）掛工作台輸入框下＋浮動面板下：
  「MinitAI boleh silap — sila semak. · AI 会犯错，请核对。 · AI can
  make mistakes — please check.」
- **章程孤兒條**：agent 依讀取順序提議「掛進 Fasal X」，人一鍵確認、
  改名留痕；「重拍那頁」降級為備選句。

### D52 — Repo 公開後的兩條鐵律：真件一律虛名；收據簽章用專用 secret（2026-09-07，122 號單 §6；依 119／121 體檢）

**背景。** repo 自 2026-09-01 起 **PUBLIC**（GitHub）。公開之前的 tree 裡有真樣本的
人名，6fa0ba4 已全部換成虛名並上線；舊名只留在 git 歷史，要不要重寫歷史等 J。

- **真人名／IC／地址／真社團名不准進 tree。** 範例、測試、截圖、eval golden case
  一律用虛名（Tan Kim Loo／Ooi Bee Huang／Chan Mei／张伟杰／王丽华／刘国华／
  Sin Hup）。讀 `.env*` 只列變數名，不看值、不貼值。這條不是「盡量」，是
  每一次 commit 前的檢查。
- **收據 QR 與章程續讀 token 的 HMAC 改用專用 `RECEIPT_SIGNING_SECRET`**
  （`src/lib/signing-secret.ts`）。以前拿 service-role key 簽：那把 key 的安全常規
  是「疑似外洩就換」，一換＝所有已印在紙上的收據 QR 變「not issued」＝把真收據
  說成假的。專用 secret 只做簽章、永不輪換；**沒設就退回 service key，什麼都
  不壞**；驗證時新舊兩把逐一試，已印的 QR 照樣驗得過。J 要做的只有在 Vercel
  與 `.env.local` 各加一行（123 號報告有一步一步）。

### D53 — 正式文件不准編：標籤要靠字掙、沒寫誰做就不准寫誰做、歧義一律舉手、Formal 卡拿掉（2026-09-07，118 號單 §0–§6；J 2026-08-31 深夜第 22 條的落地）

**病歷。** J 的理事會手稿第④條 `lanti Ajk seorg. <名字>`（委任理事一名）在正式文件裡變成
`<名字> ditugaskan untuk melantik seorang Ahli Jawatankuasa`——被委任的人寫成去委任別人的人；
第③條 `Agenda 2.1 diganti A (B)` 被寫成 `Mesyuarat memutuskan agar … digantikan kepada A`。
沒有讀錯一個字。病根是 `draft-minutes.ts` 逼每一行掛 Perbincangan／Keputusan／Tindakan 三選一，
馬來文的行動句文法上要有施事者，紙上沒有，模型就補一個。**是標籤逼它編的。**

- **標籤要靠字掙（§1-1）。** `Keputusan` 只在原文有 memutuskan／diputuskan／lulus／通过／決定／議決
  這類字；`Tindakan` 只在原文有「人名＋那個人要做的事」；其餘**不掛**，沒有第三個標籤。
  prompt 這樣寫，`src/lib/minutes-guards.ts` 的 `enforceKinds` 再用碼把沒掙到的剝掉（剝標籤永不致命）。
- **原文沒有說是誰做的，句子也不准說是誰做的（§1-2）。** `checkInventedAgent`：
  句子裡出現施事者句型（ditugaskan／diminta／被委派／is tasked…）或裁決句型（Mesyuarat
  memutuskan／会议通过…）而原文沒有對應字眼＝發明，退回模型一次（rule 7），再犯就整份退回純模板。
  原文的「不通過／延後」句子裡不准消失。單向：翻譯把「通过」寫成 disahkan 不算錯——會誤殺對的文件的檢查比沒檢查糟。
- **歧義一律舉手，不准預設選一個（§3，本場的靈魂）。** `src/lib/minutes-ambiguity.ts` 是**碼**不是 prompt，
  因為驗收是固定輸入：③④必問、①②⑤不准問（亂問一樣是病）。目前只認兩個真的害過文件的形狀
  （`lanti Ajk seorg. <名>`、`A diganti B`／`A ganti - B`），把兩種讀法寫成整句給人選，不預選、不花額度；
  人沒選之前，文件裡那一條**照原文逐字**（prompt 被告知，碼再強制）。`as_written` 記「照原文保留」。
- **Formal version／Original 對照卡整條刪（§2）。** tidy 那條線的鎖死清單搬進 draft-minutes prompt
  （錢／日期／人名整字比對／IC／決定實質／gap 留 gap）；「人名只准整字比對」的守門活著搬進
  `checkLatinNames`（`Tan Kim Looi` 不算 `Tan Kim Loo`）；「翻回原文」由文件頁的 `ItemSources` 每條接手。
  107 §8「正式版還沒存起來」那條殘**自動消失**——沒有正式版了。
- **額度全面百分比（§4）。** 「1 AI action」三語從用戶面前消失；根 layout 把 quotaPool 放進
  `AiQuotaProvider`，任何會花錢的按鈕都說「大約用本月額度的 X%」；池子未知就不說數字。
  `ui-jargon.test.ts` 用 grep 守住。
- **該抄的要抄到，不該編的不准編（§5）。** IC／地址其實一直讀到了（真件 extraction 裡就有），
  是畫面與成文層沒人印——現在核對頁有列、文件 PEMEGANG JAWATAN 印出來；`1. as` 這種碎片在
  parse 時抹掉（Hard Rule 1 准抹不准補），沒有出席名單就是沒有。
- **精靈裡 agent 在場（§6）。** 每一步結尾一句人話（「我這裡有 N 個地方不確定，現在處理嗎？」／
  「這一步我都看得懂」），舉手卡就在那句話下面；Discuss 入口改成看得見的鈕。**它只數不改**（D51 兩級制不動）。

### D54 — 出席人數由碼從人數句算、由人確認，AI 不算；人名不准音譯由碼守；帳對不上只問不改（2026-09-08，125 號單；D53 的延伸）

**病歷（J 2026-09-07 晚線上親手抓到）。** 同一張常年大會手稿，本機成品華文名保留，線上成品
`Cadangan oleh <拼音>, disokong oleh <拼音>`——三個名字被模型編成拼音，prompt 從 8/19 起就寫著 never romanised。
`attendance_count` 兩次都讀對 `理事12人,请假2人(甲,乙),会员40人`，沒有人把它變成數字；步驟 2 逼 J 打一個名字，
成品 `Jumlah hadir: 1 orang`，進 eROSES 的 Bilangan Ahli Hadir 是假的。財政九個數字讀對，7,680＋13,600−10,150＝11,130≠11,590，差 460 沒人講。

- **prompt 講過的規矩不算數，碼擋住的才算（D53 靈魂延續）。** 人名音譯就是活例子。`checkChineseNamesSurvive`
  是 `checkNames` 的反方向：詞彙表詞與「開頭是姓的日常詞」先挖掉，剩下 2–4 字姓開頭的段是人，成品裡少一個就退回模型一次
  （`ROMANISED OR DROPPED A NAME`），再犯整份退純模板（純模板照原文抄，音譜不了）。兩字段只在成品長出原文沒有的拉丁名時才算
  （周会／许可／曾任是詞不是人——誤殺對的文件比漏抓糟）。文件頭最後一道網 `droppedChineseNames` 只列不擋。
- **出席人數：碼算、人確認、AI 不算（§2）。** `parseHeadcount` 只認固定形狀（中／BM／EN 各幾種），请假／缺席／tidak hadir／apologies
  後面的數字**不算出席**、括號裡的名字進 `apologies`。步驟 2 卡問「我算出 52 人出席，對嗎？」［對］［不對，我自己填］，不預選、不預填、不花額度；
  形狀認不得＝第三個舉手形狀（`headcountQuestion`），只問「這句寫幾個人出席？」。人答的數字寫 `attendance_confirmed`——
  **它滿足 D30**（不再逼人打名字），成品 `Kehadiran: <原句>`＋`Jumlah hadir: N orang`＋`TIDAK HADIR (DENGAN MAAF)` 名單，
  eROSES 的 Bilangan Ahli Hadir 先取確認值、名單長度後備；`attendanceRecorded()` 一支函式，client 與 server action 同源。
  parse 時 `apologies` 裡的人從 `attendees` 剔掉（8/31 把兩個請假的當唯一出席者那一次）。
- **簽名欄（§3）。** 抬頭 `主席：甲　记录：乙`／`Pengerusi: … Setiausaha: …` 由碼讀（`signatoriesFromLines`，check、來源是那一行），
  prompt 只是也講一次；職位一律印文件語言的標籤（`signatureRole`：记录→SETIAUSAHA），BM 簽名欄下不印華文；
  沒人簽就是空白底線＋職位標籤，**不印 `( Pengerusi )`**（它像一個名字的格）。不准拿登入者名字冒充。
- **財政對帳（§4）。** `reconcileFigures` 整數分：上期＋收入−支出 vs 紙上結存；四樣缺一＝不查；平＝**一句話都不說**；
  不平＝核對頁一張卡兩顆鈕，**兩顆都不改數字**：「我讀錯了，我來改」只捲到那幾行；「數字沒錯，紙上本來就這樣」寫
  `figures_mismatch_noted`，成品 KEWANGAN 底下多**一行**該語言的註記。figures 多 `role`（opening／income／expense／closing／other）由模型標、碼算。
- **固定段落自動套詞彙表（§5-4）。** Tempat／KEWANGAN 描述／PEMEGANG JAWATAN 職位／PENUTUP 這些不經 AI 的段落，
  BM 成品直接跑 `applyBmGlossary`——表裡有的詞才換，人名／機構名先用私用區字元圍起來不碰。散會句與最後一條決議同文只印一次
  （比對的是**原文行**，因為模型改寫後的句子跟 verbatim 的 PENUTUP 不同字）。
- **agent 只准說它知道的（§5-1）。** 「這一步我都看得懂」改成「這一步沒有要問你的」——它數過問題等於零，就只有這麼多。
- **模型換不換是 J 看完真紙 bench 才拍板（18③）。** `scripts/bench-real-pages.ts` 每顆各讀兩次同一張真紙，
  表只寫進私密夾，.env 不動。

---

*Drafted by Minit's build assistant · 2026-07-29 · D9–D13 appended 2026-08-25 · D14–D15 appended 2026-08-25 (Stage B/C) · D16 appended 2026-08-25 (Stage D) · D17 appended 2026-08-27 (work order 27, the overnight sprint) · D18–D21 appended 2026-08-27 (work order 31 §0, J's post-launch rulings) · D22–D23 appended 2026-08-27 (work order 32 §0, launch-day feedback rulings) · D24–D25 appended 2026-08-27 (the afternoon rulings: violet redesign + BM guard) · D26–D28 appended 2026-08-27 (the launch-evening 20-point list) · D29–D32 appended 2026-08-28 (the two-review session: prompt unfreeze, attendance gate, funds page, record-to-DB) · D33–D35 appended 2026-08-28 (J's §6 answers + the new seven: PdpaNote deleted, per-part AI discussion, minutes named/printable/editable/photo-linked) · D36–D37 appended 2026-08-28 evening (the eight-item round: save lands on the finished document, saved workspaces clear themselves, AI may merge like items under checkMergedFacts) · D38–D40 appended 2026-08-28 (the design pass: one five-step radius scale shifted a notch, the canvas gradient that was being painted and covered, the four rebuilt home cards and the sign-in brand panel) · D41–D42 appended 2026-08-28 (J's review of it: no piggy bank and a standing check on imagery for every community, and one brand mark that both the page and the icon files are generated from) · D43–D44 appended 2026-08-28 night (no black buttons — the primary token is brand purple; and the free fence: lifetime 5 documents · 20 receipts · 20 pages · 3 clean downloads, watermarked previews, clean files only through counted doors) · D45 appended 2026-08-30 (work order 81: the A6 exception — a constitution upload costs the fence min(actual pages, 5), charged once per document even when read in segments) · D46 appended 2026-08-30 (work order 87: the receipt-verify QR page's three hard lines — token-only lookup, repeat only what the paper prints, never read as certifying the society) · D47–D48 appended 2026-08-30 night (work order 89: the constitution read is priced by pages — blocks of five to page 20, per page after — with the charge following the read; and the committee's eROSES-required fields hard-gate both the form and the filing, with the transliteration risk noted) · D49 appended 2026-08-30 late night (work order 94: the whole e-Invois surface goes operator-only BETA — one gate in the provider, fail-closed pages and API, the free layer follows, one BETA pill — and the partner pack's financial_resolutions field lands with every judgement in deterministic code) · D50 appended 2026-08-30 late night (work order 97: cash hand-over is to become a state on the income records, not a standalone page — the sidebar row goes now, the engine and route stay) · D51 appended 2026-08-31 (work order 100: the home page IS the agent workbench — cards deleted, loosened/locked lines written into agent-soul.ts verbatim, the two-tier change rule with a fail-closed audit trail, the standing three-language AI-mistakes line, and orphan clauses get proposed homes) · D52 appended 2026-09-07 (work order 122: the repo is public — real names never enter the tree, fictional ones only; and the receipt/continuation HMAC moves to a dedicated RECEIPT_SIGNING_SECRET that falls back to the service key when unset and verifies against both) · D53 appended 2026-09-07 (work order 118: the formal document may not invent — labels are earned by the page's words, no doer or verdict the page did not carry, two-way lines raise a hand and stay verbatim until a person chooses, the formal-version card is gone and its locks live on the document, costs read as percentages, an appointment's IC/address is printed, a name-shaped scrap is not an attendee, and the agent speaks at the end of every step without editing) · D54 appended 2026-09-08 (work order 125: a Chinese name survives in Chinese by code, the headcount line is counted by code and confirmed by a person and satisfies D30, header signatories reach the signature block with roles in the document's language, the treasurer's figures are reconciled in TypeScript and a mismatch only asks, fixed BM passages get the glossary by code, the closing line prints once, and the model is chosen only after J reads the real-page bench)*
