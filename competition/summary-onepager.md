...
Minit flips the effort. Instead of asking people to key data into forms, users photograph their records exactly as they are, and Minit drafts the compliant output. From meeting notes it produces Bahasa Malaysia minutes and a ready-to-paste eROSES Annual Return pack. From a donation ledger it builds a structured register with sequentially numbered, gap-free receipts, tracks custody of cash from collector to HQ, and generates the month-end batch file for the official MyInvois portal. It also assembles AGM packs — notice, agenda, attendance sheet, proxy forms — extracts bank resolutions, and answers constitution questions with the governing clause cited. The money side is a full book: spending is recorded from a photographed invoice or typed in, members submit reimbursement claims through a submit-approve-pay flow, and a receipts-and-payments financial statement is computed from the stored records for any period — every figure summed by code, with AI involved nowhere.
...
Where we are, stated plainly: the workflows above are built and the deterministic logic is tested. Our extraction eval currently scores 92.9% of fields (117/126) across 10 pages with 1 invented value, on `gemini-3.5-flash-lite` (re-run 24 August 2026 against the current prompt; an earlier build scored 95.2% with 0 invented on 6 August) — but those pages are synthetic typeset documents, so that figure measures reading print, not the handwritten mixed-language notes the product is for. We publish it with that limitation attached rather than let it be mistaken for a handwriting result. Our first deployments with real societies begin in August 2026, and a measured per-field accuracy figure on genuinely handwritten, mixed-language documents will be published with our artifact before judging. We would rather show judges a real number in September than an impressive claim today.
