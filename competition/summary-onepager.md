...
Where we are, stated plainly: the workflows above are built and the deterministic logic is tested. Our extraction eval currently scores 95.2% of fields (119/125) across 10 pages with 0 invented values, on `gemini-3.5-flash-lite` (re-run 29 August 2026 against the current prompt) — but those pages are synthetic typeset documents, so that figure measures reading print, not the handwritten mixed-language notes the product is for. We publish it with that limitation attached rather than let it be mistaken for a handwriting result. Our first deployments with real societies begin in August 2026, and a measured per-field accuracy figure on genuinely handwritten, mixed-language documents will be published with our artifact before judging. We would rather show judges a real number in September than an impressive claim today.
