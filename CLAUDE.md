...
10. **The assistant IS a conversation, and it is being given eyes** — see **`docs/助手重做-设计.md`**, which is newer than this rule and wins where they differ (product decision, 20 Aug 2026). Four sentences of this rule were overturned on that date; they are listed below so nobody restores them by accident.
    ...
    - ✅ **Status, 2026-08-22 — the first tool is built, and the line is gone.** `cari_minit` searches this org's CONFIRMED minutes with pgvector and the matching sections go into the prompt; `src/prompts/chat.ts` no longer says "you cannot see their records" and instead requires a `[n]` citation on every claim about a record. Every answer carries clickable sources back to the meeting (`src/components/v2/answer-sources.tsx`). Pieces: `src/lib/minutes-chunks.ts` (pure, tested) · `src/lib/ai/embed.ts` · `src/lib/ai/minutes-index.ts` (indexes on confirm) · `src/lib/ai/cari-minit.ts` (**user-scoped client — RLS is the boundary**) · `npm run embed:backfill`.
      ...
      ⚠️ **Retrieval-first, not yet model-chosen tool calls.** The design doc's shape is "the model picks a tool"; that needs function-calling across four vendors, which the provider layer does not have. Minutes are searched every turn instead. The remaining tools (donations, receipts, constitution, roster, deadlines) are still to do — §5 step 3 of the design doc.
